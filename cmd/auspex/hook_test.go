...
import (
	...
	internalhook "github.com/Sri-Krishna-V/auspex/internal/hook"
...
// `auspex hook install --agent claude --settings PATH` wires the hooks into the
...
func TestHookInstallStatusUninstallRoundTrip(t *testing.T) {
	...
	// The auspex hook commands must be gone from the file.
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read settings: %v", err)
	}
	if strings.Contains(string(data), "auspex hook ") {
		t.Errorf("auspex hooks remain after uninstall: %s", data)
	}
}
...
func TestHookManagedRequiresSingleSupportedAgent(t *testing.T) {
	...
	if code != 2 || !strings.Contains(errb, "has no auspex-managed target") {
...
func TestOpenClawDefaultInstallRefusesLegacyOnlyState(t *testing.T) {
	...
	explicitPlugin := filepath.Join(legacy, "extensions", "auspex")
	...
	if _, err := os.Stat(filepath.Join(legacy, "extensions", "auspex", "index.js")); err != nil {
	...
	// standalone Auspex invocation must continue to refuse this ambiguous root.
...
func TestHookManagedDefaultOutputUsesRuntimeHome(t *testing.T) {
	setTestHome(t, "")
	path := filepath.Join(t.TempDir(), "managed-settings.d", "auspex.json")
	...
	for _, cmd := range claudeInstalledCommands(t, path) {
		if !strings.Contains(cmd, "$HOME/.auspex/findings.ndjson") {
...
func TestExpandHookPathHandlesUnsetHome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		got, err := expandHookPath("$HOME/.auspex/records.ndjson")
		if err != nil || got != filepath.Join(home, ".auspex/records.ndjson") {
...
func TestHookInstallAllDefaultsToRecordsFile(t *testing.T) {
	...
	for _, cmd := range claudeInstalledCommands(t, path) {
		if !strings.Contains(cmd, filepath.Join(".auspex", "records.ndjson")) {
			t.Fatalf("--emit all default command = %q, want records.ndjson", cmd)
		}
		if strings.Contains(cmd, ".auspex/findings.ndjson") {
...
func TestHookManagedClaudeRoundTrip(t *testing.T) {
	...
	if code != 0 || !strings.Contains(out, "removed auspex managed hooks") {
...
func TestHookManagedCodexRoundTrip(t *testing.T) {
	...
	for _, want := range []string{"[features]", "hooks = true # auspex-managed", "[[hooks.PreToolUse]]", "--agent codex", outFile} {
	...
	if strings.Contains(testHookCommandText(readFileString(t, path)), "--installed-by=auspex") {
		t.Fatalf("managed codex uninstall left auspex hooks:\n%s", readFileString(t, path))
	}
}
...
func TestInstallRuntimeArgsKeepsRuntimeRuleDirs(t *testing.T) {
	for _, dir := range []string{"$HOME/auspex-rules", "~/auspex-rules", `~\auspex-rules`} {
...
func TestHookInstallResolvesRelativeOutputFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.json")
	relative := filepath.Join("logs", "auspex.ndjson")
	want, err := filepath.Abs(relative)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInstallRuntimeArgsKeepsRuntimeOutputPaths(t *testing.T) {
	for _, path := range []string{"$HOME/.auspex/live.ndjson", "~/.auspex/live.ndjson", `~\.auspex\live.ndjson`} {
...
func TestExpandHookPathAcceptsWindowsTildeSeparator(t *testing.T) {
	home := t.TempDir()
	setTestHome(t, home)
	got, err := expandHookPath(`~\.auspex\live.ndjson`)
	if err != nil {
		t.Fatal(err)
	}
	if want := filepath.Join(home, `.auspex\live.ndjson`); got != want {
...
func TestHookInstallOutputValidation(t *testing.T) {
	...
	_, errb, code = runCLI("hook", "install", "--agent", "claude", "--settings", path,
		"--output", "http", "--http-url", "https://user:pass@ingest.example/auspex")
	...
	_, errb, code = runCLI("hook", "install", "--agent", "claude", "--settings", path,
		"--output", "http", "--http-url", "http://ingest.example/auspex")
...
func claudeInstalledCommands(t *testing.T, path string) []string {
	...
	for _, groups := range parsed.Hooks {
		for _, group := range groups {
			for _, ref := range group.Hooks {
				...
				if strings.Contains(command, "--installed-by=auspex") {
	...
	if len(commands) == 0 {
		t.Fatalf("no auspex commands in %s: %s", path, data)
	}
	return commands
}
...
// windsurfAllEvents is the Windsurf event set auspex installs, used by
...
// `auspex hook install --agent windsurf` wires the hooks into the given
...
func TestWindsurfHookInstallStatusUninstallRoundTrip(t *testing.T) {
	...
	if strings.Contains(testHookCommandText(string(data)), "--agent windsurf") {
		t.Errorf("auspex windsurf hooks remain after uninstall: %s", data)
	}
...
// `auspex hook install --agent cursor` wires the hooks into the given hooks.json
...
func TestCursorHookInstallStatusUninstallRoundTrip(t *testing.T) {
	...
	if strings.Contains(testHookCommandText(string(data)), "--agent cursor") {
		t.Errorf("auspex cursor hooks remain after uninstall: %s", data)
	}
}
...
// `auspex hook install --agent copilot` wires the hooks into the given hooks.json
...
func TestCopilotHookInstallStatusUninstallRoundTrip(t *testing.T) {
	...
	if strings.Contains(testHookCommandText(string(data)), "--agent copilot") {
		t.Errorf("auspex copilot hooks remain after uninstall: %s", data)
	}
	if !strings.Contains(string(data), `"hooks"`) {
...
func TestVSCodeHookInstallStatusUninstallRoundTrip(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".copilot", "hooks", "auspex.json")
	...
	if strings.Contains(testHookCommandText(string(data)), "--agent copilot") {
		t.Errorf("auspex vscode hooks remain after uninstall: %s", data)
	}
	if !strings.Contains(string(data), `"hooks"`) {
...
// Every agent auspex targets now has a hook integration. Gemini in particular is a
...
func TestPrintHookReportAlignsAgentAndStatusColumns(t *testing.T) {
	...
	printHookReport(&out, "status", internalhook.InstallReport{
		...
		Supported: true,
		Message:   "auspex hooks not installed",
	})
	if got, want := out.String(), "antigravity not-installed auspex hooks not installed\n"; got != want {
...
// Sanity: the installed Claude command is a real auspex hook invocation with a
