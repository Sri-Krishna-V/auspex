// Command auspex provides endpoint visibility into AI agent activity, with
// local detection, optional pre-action blocking, and forensic reconstruction.
//
// Auspex reconstructs and inspects what an endpoint AI agent did on a machine by
...
// Subcommands:
//
//	auspex scan [--agent NAME ... | --path FILE|DIR ...]      scan agent artifacts; emit selected NDJSON records
//	auspex timeline [--agent NAME ... | --path FILE|DIR ...]  reconstruct a per-session chronological view
//	auspex collect [--addr 127.0.0.1:4318]   receive OTLP/HTTP protobuf logs; emit records
//	auspex ship --input-file F --http-url U  tail a local record file to an HTTP endpoint
//	auspex hook (install|uninstall|status)   manage live agent integrations
//	auspex agents                            report detected and supported local agents
//	auspex rules check                       validate and compile rules; run companion tests
//	auspex rules list                        list effective compiled rule ids
//	auspex rules test --fixture FILE         evaluate rules against fixture events
//	auspex case build|verify                 bundle findings, enforcement decisions, and cited events
...
// runHelp implements `auspex help <subcommand>` as a thin alias over each
...
func usage(w io.Writer) {
	fmt.Fprintln(w, `auspex — observe, detect, block, and reconstruct AI agent activity on the endpoint

usage:
  auspex scan [--agent NAME ... | --path FILE|DIR ...]      scan agent artifacts; emit records (NDJSON)
  auspex timeline [--agent NAME ... | --path FILE|DIR ...]  reconstruct a per-session chronological view (text|json)
  auspex collect [--addr 127.0.0.1:4318]   receive OTLP/HTTP protobuf logs; emit records
  auspex ship --input-file F --http-url U  tail a local record file to an HTTP endpoint
  ...
  auspex rules check                       validate and compile rules; run companion tests
  auspex rules list                        list effective compiled rule ids
  auspex rules test --fixture FILE         evaluate rules against fixture events (NDJSON)
...
Run "auspex help <command> [subcommand]" for detailed flags.`)
}
