...
func runShip(args []string, stdout, stderr io.Writer) int {
	...
	fs.Usage = func() {
		fmt.Fprintln(stderr, "usage: auspex ship --input-file PATH --http-url URL [--state-file PATH] [--poll DUR] [HTTP options]")
		fmt.Fprintln(stderr, "\nTails an append-only auspex NDJSON file to an HTTP endpoint with a durable")
	...
	defer stop()
	fmt.Fprintf(stderr, "auspex ship: shipping %s to the configured HTTP endpoint (Ctrl-C to stop)\n", *inputPath)
...
// nextRotatedShipFileID returns the oldest retained auspex record file that has
...
func writeShipCheckpoint(path string, cp shipCheckpoint) error {
	...
	b = append(b, '\n')
	f, err := os.CreateTemp(filepath.Dir(path), ".auspex-ship-state-*")
	if err != nil {
		return err
	}
	tmp := f.Name()
