...
import (
	...
	"github.com/Sri-Krishna-V/auspex/internal/casebundle"
)
...
// case.auspex evidence bundle from captured record streams, or verify one.
...
func caseUsage(w io.Writer) {
  ...
  auspex case build <case-id> --from FILE [--from FILE ...] [-o|--out DIR] [--include-raw-evidence|--include-redacted-evidence]
  auspex case verify <DIR>

build assembles a portable case.auspex bundle from previously captured NDJSON
...
func newCaseBuildFlagSet(output io.Writer, opts *caseBuildOptions) *flag.FlagSet {
	...
	fs.Usage = func() {
		_, _ = fmt.Fprintln(output, "usage: auspex case build <case-id> --from FILE [--from FILE ...] [-o|--out DIR] [--include-raw-evidence|--include-redacted-evidence]")
		fs.PrintDefaults()
	}
	return fs
}

// runCaseBuild implements `auspex case build <case-id>`. The case id is the
// first argument (mirroring `hook <event>`); flags follow.
func runCaseBuild(args []string, stdout, stderr io.Writer) int {
	if len(args) == 1 && isHelpArg(args[0]) {
		opts := caseBuildOptions{out: "case.auspex"}
		newCaseBuildFlagSet(stdout, &opts).Usage()
		return 0
	}
	if len(args) == 0 || strings.HasPrefix(args[0], "-") {
		fmt.Fprintln(stderr, "case build: the case id comes first: auspex case build <case-id> --from FILE ...")
		return 2
	}
	caseID := args[0]
...
// runCaseVerify implements `auspex case verify <dir>`. The verdict and any
...
func runCaseVerify(args []string, stdout, stderr io.Writer) int {
	if len(args) == 1 && isHelpArg(args[0]) {
		fmt.Fprintln(stdout, "usage: auspex case verify <DIR>")
		fmt.Fprintln(stdout)
		fmt.Fprintln(stdout, "Checks a case.auspex bundle against its manifest digests.")
		fmt.Fprintln(stdout, integrityNote)
		return 0
	}
	if len(args) != 1 || strings.HasPrefix(args[0], "-") {
		fmt.Fprintln(stderr, "usage: auspex case verify <DIR>")
		return 2
	}
