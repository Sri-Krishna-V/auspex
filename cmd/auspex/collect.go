...
import (
	...
	"github.com/Sri-Krishna-V/auspex/internal/otel"
...
// it via OTEL_EXPORTER_OTLP_ENDPOINT=http://127.0.0.1:4318. auspex binds the
...
// runCollect implements `auspex collect`: a long-running, in-process OTLP/HTTP
...
func runCollect(args []string, stdout, stderr io.Writer) int {
	...
	rf.register(fs)
	fs.Usage = func() {
		fmt.Fprintln(stderr, "usage: auspex collect [--addr 127.0.0.1:4318] [--emit KIND ...] [--output SINK ...] [--case-id ID] [--rules-dir DIR ...] [--no-builtin-rules]")
...
func serveCollect(ctx context.Context, addr string, rcv *collector, stderr io.Writer) int {
	...
	fmt.Fprintf(stderr, "auspex collect: listening for OTLP/HTTP logs on http://%s%s (Ctrl-C to stop)\n", ln.Addr(), otlpLogsPath)
...
func warnIfOffLoopback(addr string, stderr io.Writer) {
	...
	fmt.Fprintf(stderr, "auspex collect: WARNING: binding %s exposes an UNAUTHENTICATED OTLP receiver off-loopback; any host that can reach this address can inject telemetry. Bind 127.0.0.1 or front it with your own authentication.\n", addr)
}
