...
func TestVersion(t *testing.T) {
	...
	if !strings.Contains(out, "auspex") || !strings.Contains(out, "schema 0.2.0") {
...
func TestVersionHelp(t *testing.T) {
	for _, args := range [][]string{{"version", "--help"}, {"version", "-h"}, {"help", "version"}} {
		...
		if code != 0 || errb != "" || !strings.Contains(out, "usage: auspex version") {
...
func TestHelpDescribesOperationalBoundaries(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: []string{"WIRED means auspex-owned configuration is present", "does not verify runtime activation"},
		},
	}
...
func TestHelpSubcommandDispatch(t *testing.T) {
	...
	if errb != "" {
		t.Fatalf("auspex help scan should write help to stdout, stderr=%q", errb)
	}
	if !strings.Contains(out, "usage: auspex scan") {
		t.Fatalf("scan help should come from the scan command, got stdout=%q stderr=%q", out, errb)
	}

	out, errb, code = runCLI("help", "ship")
	if code != 0 || errb != "" || !strings.Contains(out, "usage: auspex ship") {
	...
	if errb != "" {
		t.Fatalf("auspex help rules test should write help to stdout, stderr=%q", errb)
	}
	if !strings.Contains(out, "usage: auspex rules test") {
	...
	if errb != "" {
		t.Fatalf("auspex help rules check should write help to stdout, stderr=%q", errb)
	}
	if !strings.Contains(out, "usage: auspex rules check") {
	...
	if errb != "" {
		t.Fatalf("auspex help hook install should write help to stdout, stderr=%q", errb)
	}
	if !strings.Contains(out, "usage: auspex hook install") {
