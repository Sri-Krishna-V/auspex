...
import (
	...
	"github.com/Sri-Krishna-V/auspex/internal/redact"
)
...
// runTimeline implements `auspex timeline`: discover agent artifacts, parse them
...
func runTimeline(args []string, stdout, stderr io.Writer) int {
	...
	fs.Usage = func() {
		fmt.Fprintln(stderr, "usage: auspex timeline [--agent NAME ... | --path FILE|DIR ...] [--case-id ID] [--profile evidence|full] [--format text|json]")
...
// timelineReport is the json shape of `auspex timeline --format json`: the schema
