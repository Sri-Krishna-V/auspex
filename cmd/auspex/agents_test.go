...
// supportedAgentIDs returns the sorted set that `auspex agents` must describe:
...
func TestKiroInventoryDescribesSharedIDEAndCLIv3Coverage(t *testing.T) {
	...
	for _, want := range []string{"KIRO_HOME", "IDE separately", "~/.kiro/hooks/auspex.json"} {
	...
	if got, want := kiro.wiredPath(home), filepath.Join(cliRoot, "hooks", "auspex.json"); got != want {
		t.Fatalf("Kiro wired path = %q, want selected CLI target %q", got, want)
	}
	defaultIDEHook := filepath.Join(home, ".kiro", "hooks", "auspex.json")
	if _, err := hook.Install(hook.AgentKiro, defaultIDEHook, "/opt/auspex", "", false); err != nil {
		t.Fatal(err)
	}
...
func TestKiroInventoryDoesNotHideUnreadableAlternateProfile(t *testing.T) {
	...
	cliRoot := filepath.Join(t.TempDir(), "kiro-cli-profile")
	t.Setenv("KIRO_HOME", cliRoot)
	if _, err := hook.Install(hook.AgentKiro, filepath.Join(cliRoot, "hooks", "auspex.json"), "/opt/auspex", "", false); err != nil {
		t.Fatal(err)
	}
	defaultIDEHook := filepath.Join(home, ".kiro", "hooks", "auspex.json")
...
func TestGrokInventoryHonorsGrokHome(t *testing.T) {
	...
	if got, want := grok.wiredPath("/unused"), filepath.Join(root, "hooks", "auspex.json"); got != want {
...
func TestAgentsDetectedAndWired(t *testing.T) {
	...
	if _, err := hook.Install(hook.AgentClaude, settings, "/usr/local/bin/auspex", "", false); err != nil {
...
func TestAgentsReportsOpenClawStateAndConfigRoots(t *testing.T) {
	t.Run("state root owns transcripts and plugin", func(t *testing.T) {
		...
		if _, err := hook.InstallWithOptions(hook.AgentOpenClaw, pluginDir, filepath.Join(home, "bin", "auspex"), hook.InstallOptions{Enforce: true}); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("config path does not relocate transcripts", func(t *testing.T) {
		...
		pluginDir := hook.OpenClawPluginPath(home)
		if want := filepath.Join(configRoot, "extensions", "auspex"); pluginDir != want {
			t.Fatalf("plugin path = %q, want %q", pluginDir, want)
		}
		if _, err := hook.InstallWithOptions(hook.AgentOpenClaw, pluginDir, filepath.Join(home, "bin", "auspex"), hook.InstallOptions{}); err != nil {
			t.Fatal(err)
		}
...
func TestAgentsReportsWiredGooseAndKiloUnderXDG(t *testing.T) {
	...
	if _, err := hook.InstallWithOptions(hook.AgentGoose, hook.GoosePluginPath(home), "/opt/auspex", hook.InstallOptions{}); err != nil {
		t.Fatal(err)
	}
	if _, err := hook.InstallWithOptions(hook.AgentKilo, hook.KiloPluginPath(home), "/opt/auspex", hook.InstallOptions{}); err != nil {
		t.Fatal(err)
	}
