...
// runHookAdmin handles `auspex hook install|uninstall|status [--agent ...]`. It
// wires auspex's live hook handler into each requested agent's hook integration
...
func runHookAdmin(action string, args []string, stdout, stderr io.Writer) int {
	...
	// --enforce opts this install into auspex's enforcement path: the installed
	...
	// it auspex stays strictly detection-only and passthrough-safe. Meaningful only
	// for install; ignored by uninstall/status (which key on the auspex marker).
	var (
		enforce           bool
	...
	fs.Usage = func() {
		...
		fmt.Fprintf(stderr, "usage: auspex hook %s %s [--settings PATH] [--managed]", action, agentArg)
		if action == "install" {
		...
		switch action {
		case "install":
			fmt.Fprintln(stderr, "\nInstalls auspex's live integrations in monitor-only mode by default.")
		...
		case "uninstall":
			fmt.Fprintln(stderr, "\nRemoves only auspex-owned integration configuration.")
		case "status":
			fmt.Fprintln(stderr, "\nInspects auspex-owned integration configuration; it does not verify agent trust,")
	...
	if *managedTarget {
		for _, agent := range agents {
			if !hook.AgentSupportsManagedHooks(agent) {
				fmt.Fprintf(stderr, "hook %s: --managed supports %s; %s has no auspex-managed target\n", action, hook.ManagedAgentUsage(), agent)
				return 2
			}
		}
	}
	...
	if sinks.file {
		...
		if file == "" {
			...
			if cfg.managed && findingsOnly {
				file = "$HOME/.auspex/findings.ndjson"
			} else if cfg.managed {
				file = "$HOME/.auspex/records.ndjson"
...
func settingsPathFor(agent, home string) string {
	...
	case hook.AgentGrok:
		// Project hooks (<cwd>/.grok/hooks/auspex.json) remain available via --settings.
...
// auspexBinaryPath returns the absolute path to the running auspex binary, used
// as the hook command so it fires regardless of the agent's PATH or working
// directory.
func auspexBinaryPath() (string, error) {
	exe, err := os.Executable()
	if err != nil {
		return "", fmt.Errorf("resolve auspex binary path: %w", err)
	}
	return exe, nil
}
