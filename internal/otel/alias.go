...
// auspex's vocabulary by exact event.name. It returns true when it recognized
...
// record from any other exporter — a false positive auspex's high-precision
...
// auspex's vocabulary. The caller has already confirmed ev.SourceAgent is
...
// tool_name is the only signal and there is no process.command — so auspex does
...
// (accept/auto_accept/reject/modify) onto auspex's permission decision values.
