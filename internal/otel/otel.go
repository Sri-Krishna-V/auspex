...
// telemetry into auspex's normalized model.Event. It is the OTel-sensor analog
...
// Standard semantic-convention attribute keys auspex keys off. These are
...
const (
	...
	// their event name this way, so auspex consults the attribute when the proto
...
// resolve to auspex's canonical model.Agent* id. An unrecognized service.name
// yields AgentUnknown rather than a guess — auspex does not pretend to know
...
// mapRecord classifies one OTLP log record onto auspex's event vocabulary. It
...
// semconv attributes, returning false when the record has no analog in auspex's
...
func classify(ev *model.Event, a *attrs, rec logRecord) bool {
	...
	default:
		// No standard-semconv signal auspex understands. Do not invent an
...
// permission outcome onto auspex's DecisionAllowed/DecisionDenied, returning ""
