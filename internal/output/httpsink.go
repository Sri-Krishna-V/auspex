...
// environment only (see cmd/auspex), never from a flag, and are never logged.
const (
	AuthNone   = "none"
	AuthBearer = "bearer"
...
const (
	...
	// DefaultHMACHeader and DefaultTimestampHeader are auspex's default HMAC
	// header names, exported so the CLI can use them as flag defaults. They are
	// auspex-specific names; see applyAuth.
	DefaultHMACHeader      = "X-Auspex-Signature"
	DefaultTimestampHeader = "X-Auspex-Timestamp"
)
...
// without auspex reflecting any attacker-influenced body content.
...
func (h *HTTPSink) flushLocked() error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		// otherwise reflect arbitrary content into auspex's stderr diagnostics.
...
//
// This is a conventional HMAC signing algorithm, but auspex ships its own default
// header names (X-Auspex-Signature / X-Auspex-Timestamp). Point a receiver at the
