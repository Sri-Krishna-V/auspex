...
func installClaudeManagedWithArgs(path, binary string, runtimeArgs []string, enforce bool) (InstallReport, error) {
	...
	rep.Changed = true
	if enforce {
		rep.Message = "installed auspex managed hooks (enforce mode: blocks matches from rules marked enforce=true)"
	} else {
...
const (
	codexAuspexFeatureBlockStart = "# BEGIN auspex managed codex features"
	codexAuspexFeatureBlockEnd   = "# END auspex managed codex features"
	codexAuspexHookBlockStart    = "# BEGIN auspex managed codex hooks"
	codexAuspexHookBlockEnd      = "# END auspex managed codex hooks"
	codexAuspexFeatureLine       = "hooks = true # auspex-managed"
)
...
func installCodexRequirementsWithArgs(path, binary string, runtimeArgs []string, enforce bool) (InstallReport, error) {
	...
	body, err = codexRequirementsWithAuspexHooks(body, binary, runtimeArgs, enforce)
	if err != nil {
		return rep, err
	}
	backup, err := backupIfExists(path)
	...
	rep.Changed = true
	if enforce {
		rep.Message = "installed auspex managed hooks in requirements.toml (enforce mode)"
	} else {
		rep.Message = "installed auspex managed hooks in requirements.toml"
	}
	return rep, nil
}
...
func uninstallCodexRequirements(path string) (InstallReport, error) {
	...
	body := removeAuspexCodexRequirements(string(bodyBytes))
	if body == string(bodyBytes) {
		rep.Message = "no auspex managed hooks present"
		return rep, nil
	}
	backup, err := backupIfExists(path)
	if err != nil {
	...
	rep.Changed = true
	rep.Message = "removed auspex managed hooks from requirements.toml"
	return rep, nil
}
...
func statusCodexRequirements(path string) (InstallReport, error) {
	...
	rep.Installed = strings.Contains(body, codexAuspexHookBlockStart)
	if rep.Installed {
		rep.Message = "auspex managed hooks installed"
	} else {
		rep.Message = "auspex managed hooks not installed"
...
func codexRequirementsWithAuspexHooks(body, binary string, runtimeArgs []string, enforce bool) (string, error) {
	body = removeAuspexCodexRequirements(body)
	var err error
...
func removeAuspexCodexRequirements(body string) string {
	var changed bool
	body, changed = removeMarkedBlock(body, codexAuspexHookBlockStart, codexAuspexHookBlockEnd)
	body, changed = removeMarkedBlockChanged(body, codexAuspexFeatureBlockStart, codexAuspexFeatureBlockEnd, changed)
	body, changed = removeMarkedLineChanged(body, codexAuspexFeatureLine, changed)
	if !changed {
		return body
	}
	body = strings.TrimRight(body, "\n")
...
func renderCodexRequirementsHookBlock(binary string, runtimeArgs []string, enforce bool) string {
	...
	b.WriteString(codexAuspexHookBlockStart)
	b.WriteByte('\n')
	...
	b.WriteString(codexAuspexHookBlockEnd)
	b.WriteByte('\n')
	return b.String()
}
...
func ensureCodexHooksFeature(body string) (string, error) {
	...
	if start == -1 {
		...
		body += codexAuspexFeatureBlockStart + "\n[features]\n" + codexAuspexFeatureLine + "\n" + codexAuspexFeatureBlockEnd + "\n"
		return body, nil
	}
	...
	insert = append(insert, codexAuspexFeatureLine)
