...
// ResolveLifecycle must map each OpenCode hook moment auspex wires onto the right
// auspex lifecycle. OpenCode's plugin surface exposes the whole event stream, but
// auspex wires tool, prompt, assistant, and session moments. The raw-event
...
func TestResolveOpenCodeLifecycle(t *testing.T) {
	...
	// OpenCode fires a LARGER event set than auspex wires (session.idle/updated/
	// error/compacted, command/file/permission/message events). auspex resolves ONLY
...
// resolver must map BOTH the auspex lifecycle arg the installer bakes into the
// plugin command AND the raw OpenCode event name OpenCode itself fires to the same
// auspex Lifecycle — and onward to the same model.EventType. This pins that the
// two entry paths (the plugin's `auspex hook <arg>` invocation vs the raw event)
// never drift apart for any wired lifecycle.
func TestOpenCodeInstallerResolverLifecycleParity(t *testing.T) {
	// rawFor maps each wired auspex lifecycle to the raw OpenCode event name the
	// plugin observes and forwards for it. The plugin invokes auspex with the
	...
	for _, lc := range openCodeHookLifecycles {
		// The path the installed plugin command takes: the auspex lifecycle arg.
...
// oc builds an OpenCode hook payload EXACTLY as auspex's generated plugin forwards
...
func TestMapOpenCodeEvents(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// after-event. auspex must NOT set a tool_error tag for OpenCode live events —
...
func TestMapOpenCodeCurrentNetworkTools(t *testing.T) {
	for _, tc := range []struct {
		...
		{tool: "search", args: map[string]any{"query": "auspex"}},
...
// payload by literally mirroring the keys auspex's generated plugin forwards on
...
func TestOpenCodePluginForwardsLiveShapeKeys(t *testing.T) {
	src := openCodePluginSourceWithArgs("/usr/local/bin/auspex", nil)
...
// auspex installs the OpenCode plugin OBSERVE-ONLY and the plugin never reads
// auspex's stdout, so the passthrough is the inert empty object. It must be a
// fresh map each call.
