...
func TestPiInstallLifecycleAndForeignCollision(t *testing.T) {
	path := filepath.Join(t.TempDir(), "extensions", "auspex.ts")
	rep, err := InstallWithOptions(AgentPi, path, "/opt/auspex", InstallOptions{RuntimeArgs: []string{"--output=file"}, Enforce: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := InstallWithOptions(AgentPi, path, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
	...
	if _, err := InstallWithOptions(AgentPi, path, "/opt/auspex", InstallOptions{}); err == nil {
...
func TestKimiInstallPreservesConfigAndUsesOnlyDocumentedFields(t *testing.T) {
	...
	if _, err := InstallWithOptions(AgentKimi, path, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	body := readTestFile(t, path)
	commandText := hookCommandText(body)
	if !strings.Contains(body, original) || strings.Count(body, kimiAuspexBlockStart) != 1 {
	...
	if strings.Count(readTestFile(t, path), kimiAuspexBlockStart) != 1 {
...
func TestQwenInstallPreservesUnrelatedJSON(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"theme":"dark","hooks":{"PreToolUse":[{"matcher":"Bash","hooks":[{"name":"auspex","type":"command","command":"foreign"}]}]}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if Status(AgentQwen, path).Installed {
		t.Fatal("foreign Qwen hook named auspex must not be treated as owned")
	}
	if _, err := InstallWithOptions(AgentQwen, path, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	...
	if !hasAuspexQwenHooks(gs) || len(gs.hooks["PreToolUse"]) != 2 {
	...
	gs, err = readGeminiSettings(path)
	if err != nil || hasAuspexQwenHooks(gs) || len(gs.hooks["PreToolUse"]) != 1 {
		t.Fatalf("Qwen uninstall = %#v, %v", gs.hooks["PreToolUse"], err)
	}
	if got := gs.hooks["PreToolUse"][0].Hooks[0]; got.Name != "auspex" || got.Command != "foreign" {
...
func TestClineInstallWritesOwnedExecutableHooks(t *testing.T) {
	dir := t.TempDir()
	if _, err := InstallWithOptions(AgentCline, dir, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	for _, event := range clineHookEvents {
		path := clineHookPath(dir, event.name)
		body := readTestFile(t, path)
		if !strings.Contains(body, auspexHookMarker) {
	...
	for _, goos := range []string{"linux", "windows"} {
		source := clineHookSourceForOS(goos, "/opt/auspex", clineHookEvent{name: "PreToolUse", lifecycle: "pre-tool"}, nil, true)
...
func TestClineUnixWrapperFailsOpenWhenAuspexCannotRun(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX wrapper test")
	}
	dir := t.TempDir()
	failing := filepath.Join(dir, "failing-auspex")
...
func TestClineUnixWrapperForwardsInputAndOutput(t *testing.T) {
	...
	dir := t.TempDir()
	passthrough := filepath.Join(dir, "passthrough-auspex")
...
func TestAmpInstallAndAuggieWrappers(t *testing.T) {
	ampPath := filepath.Join(t.TempDir(), "amp", "plugins", "auspex.ts")
	if _, err := InstallWithOptions(AgentAmp, ampPath, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	...
	foreign := `/tmp/auspex--installed-by=auspex-foreign.sh`
	if runtime.GOOS == "windows" {
		foreign = `C:\Tools\auspex--installed-by=auspex-foreign.ps1`
	}
	seed := settingsFile{
	...
	if _, err := InstallWithOptions(AgentAuggie, auggiePath, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	...
	if !hasAuspexAuggieHooks(sf, auggiePath) {
		t.Fatal("Auggie settings missing auspex hooks")
	}
	...
	if body := readTestFile(t, pre); !strings.Contains(body, auspexHookMarker) || !strings.Contains(body, "--enforce") {
...
func TestAuggiePartialStatusAndOrphanCleanup(t *testing.T) {
	...
	if _, err := InstallWithOptions(AgentAuggie, path, "/opt/auspex", InstallOptions{}); err != nil {
		t.Fatal(err)
	}
	...
	// A lost settings file must not strand executable auspex-owned wrappers.
...
func TestKiroInstallUsesOwnedGlobalHookFileSharedByIDEAndCLIv3(t *testing.T) {
	...
	setTestHome(t, home)
	t.Setenv("KIRO_HOME", "")
	path := filepath.Join(home, ".kiro", "hooks", "auspex.json")
	rep, err := InstallWithOptions(AgentKiro, path, "/opt/auspex", InstallOptions{Enforce: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := InstallWithOptions(AgentKiro, path, "/opt/auspex", InstallOptions{}); err == nil {
...
func TestKiroHooksPathHonorsKiroHome(t *testing.T) {
	...
	setTestHome(t, home)
	t.Setenv("KIRO_HOME", "")
	if got, want := KiroHooksPath("/home/u"), filepath.Join("/home/u", ".kiro", "hooks", "auspex.json"); got != want {
		t.Errorf("default KiroHooksPath = %q, want %q", got, want)
	}

	root := filepath.Join(t.TempDir(), "kiro profile")
	t.Setenv("KIRO_HOME", root)
	if got, want := KiroHooksPath("/unused"), filepath.Join(root, "hooks", "auspex.json"); got != want {
		t.Errorf("relocated KiroHooksPath = %q, want %q", got, want)
	}
	if note := kiroRuntimeCoverageNote(filepath.Join(root, "hooks", "auspex.json")); !strings.Contains(note, "Kiro CLI 2.13+") || !strings.Contains(note, "not documented for Kiro IDE") {
		t.Fatalf("relocated Kiro coverage note = %q", note)
	}
	if note := kiroRuntimeCoverageNote(filepath.Join(home, ".kiro", "hooks", "auspex.json")); !strings.Contains(note, "Kiro IDE 1.0.182+") || !strings.Contains(note, "separate root") || strings.Contains(note, "by Kiro CLI") {
		t.Fatalf("default IDE coverage note with KIRO_HOME set = %q", note)
	}

	t.Setenv("KIRO_HOME", filepath.Join(home, ".kiro"))
	if note := kiroRuntimeCoverageNote(filepath.Join(home, ".kiro", "hooks", "auspex.json")); !strings.Contains(note, "by Kiro CLI 2.13+") || strings.Contains(note, "separate root") {
		t.Fatalf("shared-root Kiro coverage note = %q", note)
	}
}

func TestGooseInstallUsesOwnedOpenPlugin(t *testing.T) {
	root := filepath.Join(t.TempDir(), ".agents", "plugins", "auspex")
	rep, err := InstallWithOptions(AgentGoose, root, "/opt/auspex", InstallOptions{
	...
	if err != nil || manifest.Name != "auspex" || manifest.Description != goosePluginMarker {
	...
	for _, event := range gooseHookEvents {
		...
		if !strings.Contains(command, auspexHookMarker) || !strings.Contains(command, "--output-file=/tmp/findings.jsonl") {
	...
	foreign := filepath.Join(t.TempDir(), ".agents", "plugins", "auspex")
	...
	if _, err := InstallWithOptions(AgentGoose, foreign, "/opt/auspex", InstallOptions{}); err == nil {
		t.Fatal("foreign Goose manifest should be refused")
	}

	occupied := filepath.Join(t.TempDir(), ".agents", "plugins", "auspex")
	...
	if _, err := InstallWithOptions(AgentGoose, occupied, "/opt/auspex", InstallOptions{}); err == nil {
...
func TestKiloInstallUsesOwnedGlobalPlugin(t *testing.T) {
	path := filepath.Join(t.TempDir(), ".config", "kilo", "plugin", "auspex.ts")
	rep, err := InstallWithOptions(AgentKilo, path, "/opt/auspex", InstallOptions{
	...
	for _, want := range []string{
		...
		`export default { id: "auspex", server }`,
		`"--enforce"`,
		`"--output=file"`,
	} {
		if !strings.Contains(body, want) {
	...
	if _, err := InstallWithOptions(AgentKilo, path, "/opt/auspex", InstallOptions{}); err == nil {
...
func TestOpenHandsInstallPreservesRepositoryHooks(t *testing.T) {
  ...
  "custom": {"keep": true}
	...
	rep, err := InstallWithOptions(AgentOpenHands, path, "/opt/auspex", InstallOptions{
		RuntimeArgs: []string{"--output=file"},
		Enforce:     true,
	})
	if err != nil {
	...
	for _, event := range openHandsHookEvents {
		...
		if !isAuspexHookCommand(ref.Command) || ref.Args != nil || ref.Timeout != event.timeout {
	...
	if _, err := InstallWithOptions(AgentOpenHands, path, "/opt/auspex", InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	sf, err = readSettings(path)
	if err != nil {
...
func TestOpenHandsRequiresRepositoryHooksPath(t *testing.T) {
	for _, path := range []string{"", filepath.Join(t.TempDir(), "hooks.json"), filepath.Join(t.TempDir(), ".openhands", "other.json")} {
		if _, err := InstallWithOptions(AgentOpenHands, path, "/opt/auspex", InstallOptions{}); err == nil {
...
func TestGooseAndKiloDefaultPaths(t *testing.T) {
	home := t.TempDir()
	if got, want := GoosePluginPath(home), filepath.Join(home, ".agents", "plugins", "auspex"); got != want {
		t.Fatalf("GoosePluginPath = %q, want %q", got, want)
	}
	xdg := filepath.Join(t.TempDir(), "xdg")
	t.Setenv("XDG_CONFIG_HOME", xdg)
	if got, want := KiloPluginPath(home), filepath.Join(xdg, "kilo", "plugin", "auspex.ts"); got != want {
