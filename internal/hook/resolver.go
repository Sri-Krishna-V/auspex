...
// per-agent key fallbacks. The agents auspex supports differ only
...
// auspex synthesizes a stable patch text by concatenating each edit's old and
...
// is present, so auspex never fabricates a digest over an empty edit.
...
// Gemini's tool_response object — never from a top-level key. auspex never
...
// openCodeToolInput resolves an OpenCode tool's input arguments. auspex's
...
// `{ title, output: string, metadata: M }`; auspex's generated plugin forwards
...
// agent may use into auspex's DecisionAllowed/DecisionDenied/DecisionAsked. An
...
// does not, ok is false and no diff fields are set — auspex never fabricates a
// digest.
