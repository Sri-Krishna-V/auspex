...
// auspex lifecycle argument its command invokes and an optional tool matcher.
...
// claudeHookEvents is the Claude Code hook set auspex installs. It covers
...
// auspexHookMarker identifies owned entries independently of the binary name.
// The live handler accepts it as inert provenance.
const auspexHookMarker = "--installed-by=auspex"
...
// AgentSupportsHooks reports whether an agent has a lifecycle-hook backend auspex
...
// synchronous pre-tool blocking contract that auspex implements.
...
// AgentSupportsManagedHooks reports whether auspex can write an upstream
...
// Claude uses a managed-settings.d drop-in so auspex does not rewrite a shared
...
func managedHooksPath(agent, goos, programFiles, programData string) (string, bool) {
	...
	case AgentClaude:
		switch goos {
		case "darwin":
			return filepath.Join(string(filepath.Separator), "Library", "Application Support", "ClaudeCode", "managed-settings.d", "auspex.json"), true
		case "windows":
			root := programFiles
			if root == "" {
				root = `C:\Program Files`
			}
			return filepath.Join(root, "ClaudeCode", "managed-settings.d", "auspex.json"), true
		default:
			return filepath.Join(string(filepath.Separator), "etc", "claude-code", "managed-settings.d", "auspex.json"), true
		}
	case AgentCodex:
		if goos == "windows" {
			root := programData
	...
	case AgentCopilot:
		if goos == "windows" {
			...
			return filepath.Join(root, "GitHub", "Copilot", "policy.d", "auspex.json"), true
		}
		return filepath.Join(string(filepath.Separator), "etc", "github-copilot", "policy.d", "auspex.json"), true
	case AgentGemini:
		switch goos {
		case "darwin":
...
func hookInvocationArgs(lifecycle, agent string, runtimeArgs []string, enforce bool) []string {
	args := []string{"hook", lifecycle, "--agent", agent, auspexHookMarker}
	if enforce {
...
func buildHookCommand(goos, binary, lifecycle, agent string, runtimeArgs []string, enforce bool) string {
	...
	cmd := fmt.Sprintf("%s hook %s --agent %s %s", shellQuote(binary), lifecycle, agent, auspexHookMarker)
	if enforce {
		cmd += " " + enforceFlag
	}
...
// isAuspexHookCommand reports whether a hook command was installed by auspex,
// detected by the marker auspex owns and always emits — never by the binary's
// name, so a renamed binary is still recognized.
func isAuspexHookCommand(cmd string) bool {
	if strings.Contains(cmd, auspexHookMarker) {
		return true
	}
	for _, script := range decodedHookCommands(cmd) {
		if strings.Contains(script, auspexHookMarker) {
			return true
		}
	}
	return false
...
func isAuspexHookRef(ref hookRef) bool {
	if isAuspexHookCommand(ref.Command) {
		return true
	}
	return slicesContain(ref.Args, auspexHookMarker)
}
...
func (sf settingsFile) hasAuspexHooks() bool {
	for _, groups := range sf.hooks {
		for _, g := range groups {
			for _, h := range g.Hooks {
				if isAuspexHookRef(h) {
					return true
				}
			}
		}
...
// applyAuspexHooksWithArgs sets auspex's hook entries for each lifecycle event,
// replacing any prior auspex entry so a re-install is idempotent. Non-auspex
// hooks in the same event are left untouched.
func (sf settingsFile) applyAuspexHooksWithArgs(binary string, runtimeArgs []string, enforce bool) {
...
// removeAuspexHooks strips every auspex-installed hook, dropping now-empty
// groups and now-empty event keys, and reports whether anything changed.
func (sf settingsFile) removeAuspexHooks() bool {
	changed := false
...
// stripAuspexGroups drops groups that consist solely of auspex hooks and removes
// auspex hooks from mixed groups, so applyAuspexHooks never leaves a stale
// duplicate behind.
func stripAuspexGroups(groups []hookGroup) []hookGroup {
...
func hasAuspexGroup(groups []hookGroup) bool {
	for _, group := range groups {
		for _, ref := range group.Hooks {
			if isAuspexHookRef(ref) {
				return true
			}
		}
	}
...
// into installed hooks: a single NDJSON file under auspex's own state directory
// (~/.auspex), mirroring the .auspex-bak naming the installer already uses. The
...
func DefaultFindingsPath(home string) string {
	return filepath.Join(home, ".auspex", "findings.ndjson")
}
...
func DefaultRecordsPath(home string) string {
	return filepath.Join(home, ".auspex", "records.ndjson")
}

// InstallOptions are the runtime flags baked into commands written by Install.
// RuntimeArgs are appended to every installed `auspex hook <event>` invocation
...
func installMessage(enforce bool) string {
	if enforce {
		return "installed auspex hooks (enforce mode: blocks matches from rules marked enforce=true)"
	}
	return "installed auspex hooks"
}
...
// Install wires auspex's hook entries into the agent's settings at path, using
// binary as the absolute auspex command. outputFile, when non-empty, is wired as
...
func InstallWithOptions(agent, path, binary string, opts InstallOptions) (InstallReport, error) {
	...
	sf.applyAuspexHooksWithArgs(binary, opts.RuntimeArgs, opts.Enforce)
	...
	rep.Changed = true
	if opts.Enforce {
		rep.Message = "installed auspex hooks (enforce mode: blocks matches from rules marked enforce=true)"
	} else {
...
// InstallManagedWithOptions wires auspex into an agent's admin-managed hook
...
func InstallManagedWithOptions(agent, path, binary string, opts InstallOptions) (InstallReport, error) {
	...
	if !rep.Supported {
		rep.Message = fmt.Sprintf("managed hooks are supported for %s; %s has no auspex-managed target", ManagedAgentUsage(), agent)
		return rep, nil
	}
	if opts.Enforce && !AgentSupportsEnforcement(agent) {
		return rep, fmt.Errorf("--enforce is supported for %s; %s is observe-only", EnforceAgentUsage(), agent)
...
// Uninstall removes only auspex's hook entries from the agent's settings,
// leaving every other key and hook intact. A missing file or absent auspex
...
// UninstallManaged removes auspex entries from an agent's admin-managed hook
...
// Status reports whether auspex hooks are present in the agent's settings,
...
// StatusManagedErr reports whether auspex entries are present in an
...
func StatusManagedErr(agent, path string) (InstallReport, error) {
	...
	case AgentCursor, AgentCopilot, AgentGemini, AgentWindsurf, AgentQwen, AgentAuggie:
		...
		if rep.Installed {
			rep.Message = "auspex managed hooks installed"
		} else {
			rep.Message = "auspex managed hooks not installed"
		}
		return rep, nil
	default:
		return rep, nil
...
func configReadable(agent, path string) error {
	...
	case AgentKimi:
		...
		_, err = kimiConfigWithoutAuspex(body)
		return err
	case AgentQwen:
		_, err := readGeminiSettings(path)
		return err
...
func writeFileAtomicMode(path string, data []byte, fileMode, dirMode os.FileMode) error {
	...
	tmp, err := os.CreateTemp(dir, ".auspex-settings-*.tmp")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
...
// backupSuffix is the fixed sibling suffix for the pristine pre-auspex backup.
const backupSuffix = ".auspex-bak"

// backupIfExists preserves the first pre-auspex version in a fixed sibling file.
...
func backupIfExists(path string) (string, error) {
	...
	// If auspex created the target from scratch, a later reinstall must not
	// manufacture a "pristine" backup from the already-modified file.
	if isAuspexHookCommand(string(data)) {
		return "", nil
	}
...
// LifecycleArgs returns the lifecycle argument names auspex wires for Claude,
