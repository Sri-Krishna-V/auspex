...
func TestOpenClawGeneratedPluginRuntime(t *testing.T) {
	...
	fakePath := filepath.Join(root, "fake-auspex.mjs")
	pluginPath := filepath.Join(root, "auspex-plugin.mjs")
	observePluginPath := filepath.Join(root, "auspex-observe-plugin.mjs")
	missingPluginPath := filepath.Join(root, "auspex-missing-plugin.mjs")
...
fs.appendFileSync(process.env.AUSPEX_OPENCLAW_TEST_CAPTURE,
...
`, node)
	...
	missingSource := strings.Replace(
		openClawPluginSource(filepath.Join(root, "missing-auspex"), nil, true),
		"const AUSPEX_DEADLINE_MS = 9000;",
		"const AUSPEX_DEADLINE_MS = 500;",
		1,
	)
	...
	cmd.Env = append(os.Environ(), "AUSPEX_OPENCLAW_TEST_CAPTURE="+capturePath)
	...
	if !slices.Equal(deny.Args, []string{
		"hook", "pre-tool", "--agent", "openclaw", auspexHookMarker, "--output=stdout", "--enforce",
	}) {
