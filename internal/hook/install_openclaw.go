...
const (
	...
	openClawPluginMarker = "auspex-managed OpenClaw hook plugin"
	openClawSourceMarker = "// auspex-managed OpenClaw plugin - do not edit"
...
func OpenClawPluginPath(home string) string {
	return filepath.Join(openClawConfigRoot(home), "extensions", "auspex")
}
...
func openClawDocuments(binary string, runtimeArgs []string, enforce bool) (openClawPluginManifest, openClawPackage, string) {
	...
	pkg := openClawPackage{
		Name:        "@sri-krishna-v/auspex-openclaw",
		Version:     "0.1.0",
		Description: openClawPluginMarker,
		Private:     true,
		Type:        "module",
...
func openClawPluginSource(binary string, runtimeArgs []string, enforce bool) string {
	...
	b.WriteString("// Generated by `auspex hook install --agent openclaw`; subprocess faults and the internal deadline return no block.\n")
	...
	b.WriteString("const AUSPEX_DEADLINE_MS = 9000;\n")
...
function forward(lifecycle, payload) {
  ...
  let child;
  try {
    child = spawn(AUSPEX_BIN, ["hook", lifecycle, "--agent", "openclaw", ...EXTRA_ARGS], {
...
function enforceTool(event, ctx) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = spawn(AUSPEX_BIN, ["hook", "pre-tool", "--agent", "openclaw", ...EXTRA_ARGS], {
...
export default {
  id: "auspex",
  name: "Auspex",
  description: "auspex-managed OpenClaw hook plugin",
  register(api) {
...
func isAuspexOpenClawManifestDocument(doc openClawPluginManifest) bool {
	return doc.ID == "auspex" && doc.Description == openClawPluginMarker
}

func isValidAuspexOpenClawManifest(doc openClawPluginManifest) bool {
	return isAuspexOpenClawManifestDocument(doc) && doc.Name == "Auspex" && doc.Version == "0.1.0" &&
...
func isAuspexOpenClawPackageDocument(doc openClawPackage) bool {
	return doc.Name == "@sri-krishna-v/auspex-openclaw" && doc.Description == openClawPluginMarker
}

func isValidAuspexOpenClawPackage(doc openClawPackage) bool {
	return isAuspexOpenClawPackageDocument(doc) && doc.Version == "0.1.0" && doc.Private && doc.Type == "module" &&
...
func openClawFileOwnership(root string) (manifest, pkg, source bool, err error) {
	...
	if manifestErr == nil {
		manifest = isAuspexOpenClawManifestDocument(manifestDoc)
	...
	if packageErr == nil {
		pkg = isAuspexOpenClawPackageDocument(packageDoc)
...
func openClawFileValidity(root string) (manifest, pkg, source bool, err error) {
	manifestDoc, manifestErr := readOpenClawManifest(openClawManifestPath(root))
	if manifestErr == nil {
		manifest = isValidAuspexOpenClawManifest(manifestDoc)
	} else if !os.IsNotExist(manifestErr) {
		return false, false, false, manifestErr
	}
	packageDoc, packageErr := readOpenClawPackage(openClawPackagePath(root))
	if packageErr == nil {
		pkg = isValidAuspexOpenClawPackage(packageDoc)
	} else if !os.IsNotExist(packageErr) {
		return false, false, false, packageErr
	}
...
func installOpenClawWithArgs(root, binary string, runtimeArgs []string, enforce bool) (InstallReport, error) {
	...
	if entries, err := os.ReadDir(root); err == nil {
		...
		if allMissing && len(entries) > 0 {
			return rep, fmt.Errorf("refusing to claim non-empty OpenClaw plugin directory without auspex-owned files at %s", root)
		}
	...
	for _, check := range []struct {
		...
		if !owned {
			return rep, fmt.Errorf("refusing to overwrite existing non-auspex OpenClaw plugin %s at %s", check.kind, check.path)
		}
	}
	...
	rep.Message = "installed auspex OpenClaw plugin (pin or enable trust as needed, then restart and verify the Gateway)"
	if enforce {
		rep.Message = "installed auspex OpenClaw plugin in enforce mode (pin or enable trust as needed, then restart and verify the Gateway)"
	}
	return rep, nil
}
...
func uninstallOpenClaw(root string) (InstallReport, error) {
	...
	if rep.Changed {
		_ = os.Remove(root)
		rep.Message = "removed auspex OpenClaw plugin files"
	} else {
		rep.Message = "no auspex OpenClaw plugin present"
	}
	return rep, nil
}
...
func statusOpenClaw(root string) InstallReport {
	...
	if rep.Installed {
		rep.Message = "auspex OpenClaw plugin package present and valid (runtime activation not verified)"
	} else if ownedManifest || ownedPackage || ownedSource {
		rep.Message = "partial or invalid auspex OpenClaw plugin package install"
	} else {
		rep.Message = "auspex OpenClaw plugin package not installed"
	}
	return rep
}
