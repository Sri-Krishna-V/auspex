...
const qwenHookName = "auspex--installed-by=auspex"
...
func isAuspexQwenHook(ref geminiHookRef) bool {
	return ref.Name == qwenHookName || isAuspexHookCommand(ref.Command)
}

func stripAuspexQwenGroups(groups []geminiHookGroup) []geminiHookGroup {
	out := make([]geminiHookGroup, 0, len(groups))
	for _, group := range groups {
		kept := group.Hooks[:0:0]
		for _, ref := range group.Hooks {
			if !isAuspexQwenHook(ref) {
...
func qwenHookState(gs geminiSettings) (complete, any bool) {
	...
	for _, event := range qwenHookEvents {
		...
		for _, group := range gs.hooks[event.settingsKey] {
			for _, ref := range group.Hooks {
				present = present || isAuspexQwenHook(ref)
			}
		}
...
func hasAuspexQwenHooks(gs geminiSettings) bool {
	_, any := qwenHookState(gs)
	return any
}

func removeAuspexQwenHooks(gs geminiSettings) bool {
	changed := false
...
func applyQwenHooks(gs geminiSettings, binary string, runtimeArgs []string, enforce bool) geminiSettings {
	for _, event := range qwenHookEvents {
		groups := stripAuspexQwenGroups(gs.hooks[event.settingsKey])
...
func statusQwen(path string) InstallReport {
	...
	if complete {
		...
		rep.Message = "partial auspex Qwen hook install"
	} else {
