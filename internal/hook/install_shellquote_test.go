...
const trickyBinary = "/Users/x/Library/Application Support/My Tools/it's auspex"
...
// followed by `hook`, and that the auspex marker is still present as its own word
// (so isAuspexHookCommand still matches after quoting).
func assertBinaryIsSingleArg(t *testing.T, cmd string) {
	...
	if !isAuspexHookCommand(cmd) {
		t.Errorf("isAuspexHookCommand failed on quoted command: %q", cmd)
	}
}
...
func TestWindowsCommandBuilderEncodesExactArguments(t *testing.T) {
	binary := `C:\Program Files\Auspex's\auspex.exe`
	output := `C:\Users\dev\Auspex & Logs\live.ndjson`
	url := `https://ingest.example/auspex?node=%COMPUTERNAME%&mode=full`
	...
	for _, want := range []string{
		...
		powerShellQuote(AgentCodex), powerShellQuote(auspexHookMarker),
	...
	if !isAuspexHookCommand(cmd) {
		t.Fatal("encoded Windows command was not recognized as auspex-owned")
	}
	config := `{"hooks":{"PreToolUse":[{"hooks":[{"command":` + fmt.Sprintf("%q", cmd) + `}]}]}}`
	if !isAuspexHookCommand(config) {
	...
	foreignFirst := fmt.Sprintf(`{"foreign":%q,"auspex":%q}`, foreign, cmd)
	if !isAuspexHookCommand(foreignFirst) {
		t.Fatal("auspex marker after a foreign encoded command was not recognized")
	}
}
...
func TestInstallShellQuotesBinaryEndToEnd(t *testing.T) {
	...
	t.Run("claude", func(t *testing.T) {
			...
			for _, g := range groups {
				for _, h := range g.Hooks {
					if isAuspexHookRef(h) {
						...
						if len(h.Args) < 2 || h.Args[0] != "hook" || !slicesContain(h.Args, auspexHookMarker) {
		...
		if n == 0 {
			t.Fatal("no auspex commands found to check")
		}
	...
	t.Run("vscode", func(t *testing.T) {
		path := filepath.Join(t.TempDir(), "hooks", "auspex.json")
...
// assertEveryFlatCommand shell-checks the command on each auspex entry in a
...
func assertEveryFlatCommand(t *testing.T, hooks map[string][]json.RawMessage) {
	...
	for _, entries := range hooks {
		for _, raw := range entries {
			...
			if cmd != "" && isAuspexHookCommand(cmd) {
				n++
