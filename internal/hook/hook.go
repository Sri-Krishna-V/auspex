// Package hook maps an endpoint agent's lifecycle hook payload into auspex's
// normalized model.Event. A hook fires live, inside the running agent, and
// hands auspex a JSON object on stdin describing what the agent is about to do
...
import (
	...
	"github.com/Sri-Krishna-V/auspex/internal/applypatch"
...
// Lifecycle is the normalized positional event accepted by `auspex hook`.
...
// resolveJunieEvent is limited to the callbacks auspex installs. Junie's
...
// resolveClaudeEvent accepts the lifecycles auspex installs plus the raw Claude
...
// resolveAntigravityEvent accepts the events and aliases auspex installs.
...
// resolveFactoryEvent accepts the documented Factory events auspex installs.
...
// resolveGrokEvent accepts the Grok events auspex installs and their
...
// resolveHermesEvent accepts the Hermes events auspex installs and their
...
// resolveCodexEvent accepts the Codex events auspex installs and their
// installer-written lifecycle names. Compaction hooks are intentionally omitted:
// auspex has no compaction event type and must not mislabel them as sessions.
...
// auspex's installer-written lifecycle aliases.
...
// hook file onto auspex lifecycles.
...
// resolveWindsurfEvent accepts the documented Cascade event names and auspex's
...
// aliases, and auspex's normalized lifecycle aliases. It deliberately does not
...
func Map(lc Lifecycle, agent, sourceAgent, eventID string, payload map[string]any) model.Event {
	...
	// events auspex installs produce native camelCase payloads in Copilot CLI;
	...
	case LifecycleFileRead:
		...
		// keeping with auspex's evidence-not-content philosophy.
...
// activity families auspex reasons about. Copilot names its built-in tools
...
// tool names into the activity families auspex reasons about. The set is
// intentionally small and explicit: if a tool name is not one of the observed
// VS Code/Copilot spellings below, auspex emits a generic tool.call/result
...
// moments onto auspex's event vocabulary. VS Code payloads are close to the
...
// surface, so auspex retains the reported tool without inventing a server.
...
// families auspex reasons about. Codex surfaces use Bash, shell, shell_command,
...
// families auspex reasons about. Gemini names its built-in tools differently from
...
// activity families auspex reasons about. OpenCode names its built-in tools with
...
func classifyOpenCodeHookTool(ev *model.Event, r *resolver, post bool) {
	...
	case "write":
		...
		// its metadata, so file.write records the path only — auspex never hashes the
...
// classifyTool maps a pre-tool invocation onto auspex's event vocabulary using
...
// allow/deny decision is hook output, not stdin input, so auspex does not read
