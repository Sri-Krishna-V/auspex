...
// ResolveLifecycle maps the Codex events auspex installs. Compaction events are
// deliberately rejected because they do not map to a auspex event type.
...
// settingsKey, e.g. "SubagentStop") AND the auspex lifecycle arg the installer
// writes into the command (the lifecycle field, e.g. "session-end") to the same
// auspex Lifecycle — and onward to the same model.EventType. The installer wires
