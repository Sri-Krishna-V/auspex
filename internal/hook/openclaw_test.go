...
func TestOpenClawInstallPackageLifecycle(t *testing.T) {
	root := filepath.Join(t.TempDir(), "extensions", "auspex")
	rep, err := InstallWithOptions(AgentOpenClaw, root, "/opt/Auspex Bin/auspex", InstallOptions{
		RuntimeArgs: []string{"--emit=findings,events", "--output=file", "--output-file=/tmp/auspex.ndjson"},
		Enforce:     true,
	})
	if err != nil {
		t.Fatal(err)
	...
	if !isAuspexOpenClawManifestDocument(manifest) || !manifest.Activation.OnStartup ||
	...
	if !isValidAuspexOpenClawManifest(manifest) || !isValidAuspexOpenClawPackage(pkg) {
	...
	if _, err := InstallWithOptions(AgentOpenClaw, root, "/opt/Auspex Bin/auspex", InstallOptions{Enforce: true}); err != nil {
	...
	if _, err := InstallWithOptions(AgentOpenClaw, root, "/opt/auspex", InstallOptions{}); err != nil {
...
func TestOpenClawInstallerRefusesForeignFiles(t *testing.T) {
	root := filepath.Join(t.TempDir(), "auspex")
...
func TestOpenClawPluginPathHonorsStateDir(t *testing.T) {
	...
	home := t.TempDir()
	if got, want := OpenClawPluginPath(home), filepath.Join(home, ".openclaw", "extensions", "auspex"); got != want {
	...
	if got, want := OpenClawPluginPath("/unused"), filepath.Join(root, "extensions", "auspex"); got != want {
		t.Fatalf("overridden OpenClawPluginPath = %q, want %q", got, want)
	}

	t.Setenv("OPENCLAW_STATE_DIR", "")
	t.Setenv("OPENCLAW_HOME", "~/service")
	if got, want := OpenClawPluginPath(home), filepath.Join(home, "service", ".openclaw", "extensions", "auspex"); got != want {
		t.Fatalf("OPENCLAW_HOME OpenClawPluginPath = %q, want %q", got, want)
	}

	t.Setenv("OPENCLAW_STATE_DIR", "~/state")
	if got, want := OpenClawPluginPath(home), filepath.Join(home, "service", "state", "extensions", "auspex"); got != want {
...
func TestOpenClawPluginPathHonorsConfigRootPrecedence(t *testing.T) {
	...
	wantDefault := filepath.Join(home, ".openclaw", "extensions", "auspex")
	...
	wantConfig := filepath.Join(filepath.Dir(config), "extensions", "auspex")
	...
	t.Setenv("OPENCLAW_CONFIG_PATH", "")
	wantUnprofiled := filepath.Join(home, ".openclaw", "extensions", "auspex")
	...
	t.Setenv("OPENCLAW_STATE_DIR", state)
	wantState := filepath.Join(state, "extensions", "auspex")
