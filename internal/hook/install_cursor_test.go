...
// countCursorAuspexHooks counts auspex-installed entries across all events.
func countCursorAuspexHooks(hooks map[string][]json.RawMessage) int {
	n := 0
...
func TestCursorInstallCreatesAllEvents(t *testing.T) {
	...
	rep, err := Install(AgentCursor, path, auspexBinary, "", false)
	if err != nil {
	...
	if got, want := countCursorAuspexHooks(hooks), len(cursorHookEvents); got != want {
		t.Fatalf("installed %d auspex hooks, want one per event (%d)", got, want)
	}
...
func TestCursorInstallIsIdempotent(t *testing.T) {
	...
	if _, err := Install(AgentCursor, path, auspexBinary, "", false); err != nil {
	...
	hooks := readCursorHooks(t, path)
	if got, want := countCursorAuspexHooks(hooks), len(cursorHookEvents); got != want {
		t.Fatalf("after two installs got %d auspex hooks, want %d (no duplicates)", got, want)
	}
}
...
func TestCursorReinstallMigratesOwnedSpecializedHooks(t *testing.T) {
	...
	seed := map[string]any{
		...
		"hooks": map[string]any{
			"beforeShellExecution": []any{
				map[string]any{"command": cursorCommandWithArgs(auspexBinary, "beforeShellExecution", nil, false)},
				map[string]any{"command": "/opt/user-audit shell"},
			},
			"afterFileEdit": []any{
				map[string]any{"command": cursorCommandWithArgs(auspexBinary, "afterFileEdit", nil, false)},
			},
		},
	}
	...
	hooks := readCursorHooks(t, path)
	if got := countCursorAuspexHooks(hooks); got != len(cursorHookEvents) {
		t.Fatalf("auspex hooks = %d, want %d", got, len(cursorHookEvents))
	}
...
func TestCursorEnforceInstallWiresOnlyBlockingHooks(t *testing.T) {
	path := filepath.Join(t.TempDir(), "hooks.json")
	if _, err := InstallWithOptions(AgentCursor, path, auspexBinary, InstallOptions{Enforce: true}); err != nil {
		t.Fatal(err)
	}
	cs, err := readCursorSettings(path)
	if err != nil {
	...
	if got, want := countCursorAuspexHooks(cs.hooks), len(cursorHookEvents); got != want {
	...
	if _, err := InstallWithOptions(AgentCursor, path, auspexBinary, InstallOptions{}); err != nil {
		t.Fatal(err)
	}
	cs, err = readCursorSettings(path)
	if err != nil {
	...
	for _, entries := range cs.hooks {
		for _, raw := range entries {
			if entryIsAuspex(raw) && strings.Contains(decodedHookCommand(rawEntryCommand(raw)), enforceFlag) {
...
func TestCursorInstallPreservesUnrelatedKeysAndHooks(t *testing.T) {
	...
	// beforeShellExecution hook auspex must not disturb. A user entry may carry
	...
	// The user's audit hook must still be present alongside auspex's, with its
	...
	if countCursorAuspexHooks(cs.hooks) != len(cursorHookEvents) {
		t.Errorf("auspex hooks = %d, want %d", countCursorAuspexHooks(cs.hooks), len(cursorHookEvents))
	}
}

func TestCursorUninstallRemovesOnlyAuspexHooks(t *testing.T) {
	path := filepath.Join(t.TempDir(), "hooks.json")
	seed := map[string]any{
		"version": 1,
		"hooks": map[string]any{
	...
	if countCursorAuspexHooks(cs.hooks) != 0 {
		t.Errorf("auspex hooks remain after uninstall: %+v", cs.hooks)
	}
...
// Uninstalling from a file that held ONLY auspex entries must still leave a
...
func TestCursorUninstallLeavesValidSchema(t *testing.T) {
	path := filepath.Join(t.TempDir(), "hooks.json")
	// Start from nothing: install writes only auspex entries.
	if _, err := Install(AgentCursor, path, auspexBinary, "", false); err != nil {
		t.Fatalf("Install: %v", err)
	}
	...
	if countCursorAuspexHooks(readCursorHooks(t, path)) != 0 {
...
// Detection keys on the auspex marker, not the binary path, so a binary with no
// "auspex" in its name still installs, reports installed, uninstalls cleanly, and
...
func TestCursorInstallDetectionIsRenameProof(t *testing.T) {
	...
	if got := countCursorAuspexHooks(readCursorHooks(t, path)); got != len(cursorHookEvents) {
	...
	if got := countCursorAuspexHooks(readCursorHooks(t, path)); got != 0 {
		t.Errorf("hooks remain after uninstall: %d", got)
	}
}

// The backup must preserve the pristine pre-auspex original across repeated
...
func TestCursorInstallWiresDurableOutputFile(t *testing.T) {
	...
	if _, err := Install(AgentCursor, path, auspexBinary, out, false); err != nil {
		t.Fatalf("Install: %v", err)
	}
	cs, err := readCursorSettings(path)
	if err != nil {
	...
	if n != len(cursorHookEvents) {
		t.Fatalf("wired %d auspex commands, want %d", n, len(cursorHookEvents))
	}
}
