...
// file through the matching flavor extractor. A flavor auspex does not parse is
...
// file carried activity auspex could not read; a file of only over-long
...
func (e OpenClawExtractor) mapNativeLine(res *Result, src Source, sha string, st *openClawState, line int, raw []byte) {
	...
	case openClawTypeCustomMessage:
		...
		// envelope. auspex recognizes it and intentionally no-ops it: it never
...
func (e OpenClawExtractor) mapMessage(res *Result, src Source, sha string, st *openClawState, line int, entry *openClawEntry) {
	...
	for i := range entry.Message.Content {
		...
		default:
			...
			// auspex models yet; ignore quietly to avoid diagnostic noise.
		}
	}
}
...
// deliberately NARROW and artifact-backed: only the OpenClaw tool names auspex
