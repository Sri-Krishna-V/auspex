...
func (e OpenCodeExtractor) Extract(r io.Reader, src Source) (*Result, error) {
	...
	if trimmed[0] != '{' {
		...
		// (a bare array, a scalar, JSONL, truncated bytes) is not a shape auspex
	...
	default:
		...
		// (a session info file, or an unrecognized record) carries no event auspex
...
func (e OpenCodeExtractor) extractMessage(res *Result, src Source, sha string, message openCodeMessage) {
	...
	default:
		// system/synthetic or a future role: not a turn auspex attributes to a
...
// (no event, no preview) so auspex never retains assistant prose or model
// reasoning. An unmodeled part kind records a diagnostic so coverage never
// silently drops a part auspex is expected to map.
func (e OpenCodeExtractor) extractPart(res *Result, src Source, sha string, trimmed []byte) {
	...
	default:
		...
		// subtask part carries no forensic signal auspex maps. Surface the gap
