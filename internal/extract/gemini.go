...
//     result), or a kind auspex does not model.
//
...
func (e GeminiExtractor) extractLogs(res *Result, src Source, sha string, trimmed []byte) {
	...
	for i := range entries {
		...
		if en.Type != geminiLogEntryUser {
			...
			// transcript shape auspex models. Skip it rather than guess a role.
			continue
		}
...
// by the top-level JSON token so auspex is robust across versions. Events from
...
func (e GeminiExtractor) extractCheckpoint(res *Result, src Source, sha string, trimmed []byte) {
	...
	default:
		// Not an object or array: not a checkpoint shape auspex can trust (a
	...
	for msgIdx := range msgs {
		...
		for partIdx := range m.Parts {
			...
			default:
				...
				// forensic value auspex maps, and guessing would risk a false event.
			}
		}
	}
}
