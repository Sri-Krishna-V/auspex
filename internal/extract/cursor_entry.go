...
// fields auspex maps are pulled out; anything else is ignored so an unrelated
...
// error flag are read only when the record carries them structurally; auspex
...
func (c *cursorContent) UnmarshalJSON(b []byte) error {
	...
	case '[':
		...
		return nil
	default:
		// A non-string, non-array content (e.g. an object) carries no body auspex
...
// since auspex's Timestamp field is RFC3339 and a bare epoch number is not one.
...
// numeric epoch returns "" (see time() for why auspex does not guess a format).
