...
// the JSONL files contain the event stream auspex needs. The parser accepts only
...
func (e CopilotExtractor) mapLine(res *Result, src Source, sha string, st *copilotState, line int, raw []byte) {
	...
	if !hasMsg && !hasTool {
		...
		// signal auspex maps; only a genuinely unknown kind is a diagnostic.
...
// kind). Copilot fires session/compact lifecycle records auspex does not map.
...
// classifyCopilotTool maps a Copilot tool invocation onto auspex's normalized
