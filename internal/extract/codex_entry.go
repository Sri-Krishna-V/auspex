...
// the variants auspex maps are named; unknown variants fall through to a
...
// Codex event_msg types auspex interprets: explicit user prompts, distinct
...
// Tags auspex stamps on Codex events. patch_apply_requested marks an
...
// separately in a patch_apply_end event_msg, and auspex does not correlate the
...
// working directory, and runtime details. Only fields auspex uses are decoded.
...
// written before each turn of a resumed session; auspex uses cwd to keep
...
// variant fields auspex reads. Codex has written arguments/input as both JSON
...
type codexResponseItem struct {
	...
	// not a free-form "output" string. auspex surfaces the discovered tool
...
// (url + pattern) variants; auspex surfaces whichever locator is present so a
...
// codexEventMsg is the tolerant view of the event_msg variants auspex uses.
type codexEventMsg struct {
	...
	// codexMcpResultIsError; left raw here so a shape auspex does not recognize is
...
// its shape is unrecognized, so a tool whose end-event auspex cannot read is
...
//	*** End Patch
//
// The header keys carry the paths; auspex returns them in order so a reviewer
