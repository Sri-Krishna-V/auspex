...
// flavor-specific extractor. auspex mirrors that contract — it MUST NOT coerce a
...
//
//	flavor      detect signal                                  auspex support
//	--------    -------------------------------------------    ----------------------------
...
//     produces the tag.
//   - Native exec results expose details.exitCode; auspex emits it only when it
...
// openClawFlavor is the per-file wire format auspex detected from the decoded
...
const (
	...
	// agent_message). auspex recognizes but does not parse it: it emits a
...
// Only the kinds auspex maps or explicitly ignores are named; any other type is
...
const (
	...
	// context as a user-side custom message, not an agent action. auspex
	...
	// which auspex recognizes but does not parse.
...
// Auspex reads only the fields it maps onto a typed event; everything else in
// input is ignored.
const (
...
// auspex maps are decoded; OpenClaw entries carry many more (id, parentId,
// timestamps, token bookkeeping) that auspex does not surface, so unrelated
...
type openClawEntry struct {
	...
	// auspex recognizes it and no-ops it (it is context replay, not an action),
...
func (m *openClawMessage) UnmarshalJSON(b []byte) error {
	...
	default:
		...
		// provider-native shape auspex trusts; leave content empty so the line
