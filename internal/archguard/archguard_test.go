...
// ---------------------------------------------------------------------------
// A2b — cmd/auspex file-level guard (AST)
// ---------------------------------------------------------------------------
...
// classifyCmdFile assigns a non-test cmd/auspex source file to exactly one of
...
func TestCmdAuspexFileSeam(t *testing.T) {
	dir := filepath.Join(moduleRoot(t), "cmd", "auspex")
	entries, err := os.ReadDir(dir)
	if err != nil {
		t.Fatalf("read cmd/auspex: %v", err)
	}
	...
	for _, e := range entries {
		...
		if !ok {
			t.Errorf("cmd/auspex/%s is unclassified; add it to the ir/mon/common/bridge classification in classifyCmdFile", name)
			continue
		}
		for _, imp := range fileImports(t, fset, filepath.Join(dir, name)) {
			...
			for _, bad := range forbidden {
				if (isInternal && leafMatches(leaf, bad)) || imp == bad {
					t.Errorf("cmd/auspex/%s imports %q, which crosses its plane seam (forbidden: %s)", name, imp, strings.Join(forbidden, ", "))
				}
			}
		}
	}
...
// requires auspex is allowed to carry. The "no cgo / SQLite / new heavy deps"
...
func TestDependencyAllowlist(t *testing.T) {
	...
	for _, path := range strings.Fields(out) {
		...
		if strings.Contains(path, "sqlite") {
			t.Errorf("direct dependency %q is a SQLite driver; auspex is cgo-free and stores state in bbolt", path)
		}
	}
		...
		err := filepath.WalkDir(dir, func(path string, d os.DirEntry, err error) error {
			...
			for _, imp := range fileImports(t, fset, path) {
				if imp == "C" {
					...
					t.Errorf("file %s imports \"C\"; auspex is cgo-free and must build with CGO_ENABLED=0", rel)
				}
			}
			return nil
		})
...
func TestThirdPartyLicensesCoverProductionModules(t *testing.T) {
	...
	dec := json.NewDecoder(strings.NewReader(goList(t, root, "-deps", "-json", "./cmd/auspex")))
