...
func TestVerifyRejectsTraversalManifest(t *testing.T) {
	...
	bundle := filepath.Join(parent, "case.auspex")
	...
	m := Manifest{SchemaVersion: "0.2.0", CaseID: "c", Tool: "auspex", Files: []ManifestFile{
...
func TestValidateManifestEvidenceMetadata(t *testing.T) {
	...
	base := func(mode string, evidence ManifestFile) Manifest {
		...
		return Manifest{
			...
			Tool: "auspex", ToolVersion: "test", EvidenceMode: mode, Files: files,
		}
	}
	tests := []struct {
		name string
...
func TestVerifyRejectsUnknownManifestField(t *testing.T) {
	dir := t.TempDir()
	raw := `{"schema_version":"0.2.0","case_id":"c","created_at":"2026-06-10T12:00:00Z","tool":"auspex","tool_version":"test","files":[],"surprise":true}`
