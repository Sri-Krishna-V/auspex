...
func TestBuildBundleShape(t *testing.T) {
	...
	opts := buildOpts(src)
	opts.Out = filepath.Join(t.TempDir(), "case.auspex")
	...
	if m.CaseID != "case-1" || m.SchemaVersion != "0.2.0" || m.Tool != "auspex" || m.ToolVersion == "" || m.EvidenceMode != "none" {
...
func TestPublishBundleDoesNotExposePartialDestination(t *testing.T) {
	root := t.TempDir()
	out := filepath.Join(root, "case.auspex")
	...
	matches, err := filepath.Glob(filepath.Join(root, ".auspex-case-*.tmp"))
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) != 0 {
...
func TestBuildDeterministic(t *testing.T) {
	...
	a := filepath.Join(t.TempDir(), "a.auspex")
	b := filepath.Join(t.TempDir(), "b.auspex")
	build(a)
	build(b)
...
func TestPublishBundleDoesNotReplaceRacingDestination(t *testing.T) {
	out := filepath.Join(t.TempDir(), "case.auspex")
	err := publishBundle(out, func(stage string) error {
...
func TestBuildSkipsIncompatibleRecords(t *testing.T) {
	...
	opts.Out = filepath.Join(t.TempDir(), "old-schema.auspex")
	res, err := Build(opts)
	...
	opts = buildOpts(src)
	opts.Out = filepath.Join(t.TempDir(), "incompatible-events.auspex")
	res = mustBuild(t, opts)
	if res.Events != 0 {
...
func TestBuildSkipsRecordsWithEmptyIDs(t *testing.T) {
	...
	opts.Out = filepath.Join(t.TempDir(), "empty-finding.auspex")
	res, err := Build(opts)
	if err == nil || !strings.Contains(err.Error(), "no findings") {
		t.Fatalf("Build error = %v, want no-findings error", err)
	}
	...
	opts = buildOpts(src)
	opts.Out = filepath.Join(t.TempDir(), "empty-event.auspex")
	res = mustBuild(t, opts)
	if res.Events != 0 {
