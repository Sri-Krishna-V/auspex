...
<https://github.com/Sri-Krishna-V/auspex/security/advisories/new>
...
auspex processes untrusted, highly sensitive agent artifacts and live payloads.
...
- Artifact scans and timelines are read-only. auspex never executes an agent,
...
- auspex makes no outbound request unless an operator configures an HTTP sink or
...
- A decoding error, relevant evaluation error, or output failure suppresses
  auspex's deny response. Unavailable sequence state prevents a sequence-based
...
- Hook input is limited to 4 MiB. Oversized or malformed input produces a
  diagnostic and no auspex deny response.
- `hook status` verifies auspex-owned configuration on disk; it does not prove
