...
Each auspex rule is one YAML file with [CEL](https://cel.dev/) boolean
...
```sh
auspex rules check --no-builtin-rules --rules-dir ./my-rules
```
...
Each rule owns its version. It is independent of auspex's record
`schema_version`; bump it whenever rule logic changes, including a
false-positive fix. auspex does not impose a versioning scheme.
...
actions are specialized the same way. `tool.call` is the fallback when auspex
...
otherwise auspex infers the dialect from command syntax. Set `tool_name` in a
...
bound, auspex emits a diagnostic. Proven independent commands remain available
...
`auspex rules check` automatically runs an adjacent companion with the same
...
hook callbacks persist their bounded window in `$HOME/.auspex/state.db`
...
auspex walks each directory recursively in path order and skips companion test
files.

```sh
auspex scan --rules-dir ./team-rules --rules-dir ./host-rules
auspex rules list --rules-dir ./team-rules
auspex rules list --no-builtin-rules --rules-dir ./team-rules
```
...
workspaces and read-only to the monitored user when possible. auspex validates
rules but does not verify signed rule bundles:

```sh
auspex hook install --agent claude --rules-dir /etc/auspex/rules
```
...
empty `--rules-dir` paths fail the command. Use `auspex rules check` in CI
