...
- **Hooks** — `auspex hook` handles a lifecycle-hook payload on stdin, so an
  action can be detected as it happens and, in enforce mode, eligible actions
  can be blocked before they run. Covered below.
- **OTLP/HTTP** — `auspex collect` runs an in-process receiver for agents that
...
`hook install` wires auspex into an agent's hook integration (a settings hook,
standalone hook file, or generated JS/TS plugin/extension). Installation is
idempotent and `uninstall` removes only auspex-owned entries or files. Installers
...
auspex hook install --agent cursor
auspex hook install --agent windsurf
...
`auspex hook install --help` for the accepted agent names.
...
OpenClaw installation writes only auspex's package; it does not run OpenClaw or
...
| Goal | How |
|---|---|
| Default user-level install | `auspex hook install --agent codex` |
| Install every supported default path | `auspex hook install --agent all` |
| Explicit path (project, alternate home, or staging) | `auspex hook install --agent codex --settings /path/to/.codex/hooks.json` |
| Explicit OpenClaw package directory | `auspex hook install --agent openclaw --settings /gateway-state/extensions/auspex` |
| OpenClaw workspace development package | `auspex hook install --agent openclaw --settings /repo/.openclaw/extensions/auspex` |
| Managed/system path | `auspex hook install --agent codex --managed` |

`--settings PATH` selects one agent-specific file or directory and therefore
must be combined with `--agent`; auspex rejects `--settings` without `--agent`,
...
OS-specific command and absolute auspex binary path; use the deployment guide's
...
and applicable enforcement decisions to `$HOME/.auspex/findings.ndjson`. When
events or indicators are selected, the default becomes
`$HOME/.auspex/records.ndjson`. Change it with
...
buffering, but no disk spool. auspex does not rotate output files itself; for
...
`AUSPEX_HTTP_TOKEN` or `AUSPEX_HTTP_HMAC_KEY` from the agent's runtime
...
For hook formats with a native timeout field, auspex writes an explicit finite
...
`--http-timeout`, which bounds one HTTP request inside auspex. Prefer file output
...
auspex hook install --agent claude --output file --output http --http-url https://ingest.example/auspex
```

PowerShell uses the same flags:

```powershell
.\auspex.exe hook install --agent claude --emit all
.\auspex.exe hook install --agent codex --emit all
```
...
only that auspex-owned configuration is present and readable.

For remote, cloud, or SSH sessions, install auspex where the agent process runs
or route supported telemetry to `collect`. Use the same agent-root environment
variables for install, status, and runtime. auspex accepts only strict JSON when
...
In default **monitor** mode, auspex never denies and preserves the host's normal
permission flow. `--enforce` allows a block only for an enforce-capable
pre-action callback and a matching rule with `enforce: true`. auspex has no
...
pre-action callback also records whether auspex computed `deny` or
...
no auspex deny response and is reported as a diagnostic.
...
the agent exposes structured payloads. auspex maps only fields present in the
...
retention tooling. Where the host has no such shipper, `auspex ship` is an
