...
auspex supports macOS, Linux, and native Windows endpoints.

## Choose a mode

| Goal | Command | Install required |
|---|---|---|
| Forensic review of existing agent activity | `auspex scan` / `auspex timeline` | no |
| Live hook capture | `auspex hook install --agent ...` | yes, per agent/user/path; managed where supported |
| Live OTLP/HTTP capture | `auspex collect` | yes, configure the agent exporter |
| Async HTTP delivery with no external shipper | `auspex ship` | no; runs as a long-lived process |
| Local inventory | `auspex agents --all` | no |
...
launches a short-lived `auspex hook ...` callback. `ship` is an optional,
...
auspex scan
auspex agents --all
auspex hook install --agent codex
auspex hook install --agent claude
auspex hook install --agent openclaw
```

Installed hooks default to a durable local file:

```text
~/.auspex/findings.ndjson
```
...
`no_override` value is auspex's computed decision, not proof that the control
response was delivered or honored by the host.

Use `--emit all --output-file ~/.auspex/live.ndjson` when you want the complete
...
auspex hook install --agent claude \
  ...
  --output file \
  --output http \
  --output-file ~/.auspex/live.ndjson \
  --http-url https://ingest.example/auspex
```

Direct HTTP is not a disk queue. If the endpoint is down, auspex reports delivery
...
Where the host has no external shipper, `auspex ship` is an optional native
...
each artifact on the target operating system with the same auspex release at
...
1. `$OPENCLAW_STATE_DIR/extensions/auspex/`
2. the directory containing `$OPENCLAW_CONFIG_PATH`, plus
   `extensions/auspex/`
3. `${OPENCLAW_HOME:-~}/.openclaw/extensions/auspex/`

Run auspex as the same operating-system user and with the same path environment
...
`OPENCLAW_STATE_DIR` to auspex or use an explicit `--settings` package
...
use `--settings` and pin that exact directory in `plugins.load.paths`. auspex
...
`<workspace>/.openclaw/extensions/auspex/`. Use
`--settings <workspace>/.openclaw/extensions/auspex` only for an intentional
...
```bash
auspex hook install --agent openhands \
...
|---|---|---|---|
| Claude Code | macOS: `/Library/Application Support/ClaudeCode/managed-settings.d/auspex.json`; Linux: `/etc/claude-code/managed-settings.d/auspex.json` | `C:\Program Files\ClaudeCode\managed-settings.d\auspex.json` | Claude uses one active managed delivery tier; server, MDM/OS, and file tiers do not merge. Check `/status` and place the hooks in the tier your fleet actually activates. |
...
| Copilot CLI | `/etc/github-copilot/policy.d/auspex.json` | `C:\ProgramData\GitHub\Copilot\policy.d\auspex.json` | Policy hooks are machine-wide and cannot be disabled by user or repository `disableAllHooks` settings. Protect both the policy file and binary. |
...
A mandatory managed auspex hook and an exclusive hook policy are different.
...
Codex's `allow_managed_hooks_only=true` separately; auspex does not set either
automatically. Installing a mandatory auspex sensor does not itself prohibit
other hooks.
...
```bash
sudo auspex hook install --agent codex --managed --emit all \
  --output-file '$HOME/.auspex/live.ndjson'
```
...
and, with `allowManagedHooksOnly`, suppress user and project hooks; auspex does
...
not documented as local source-plugin directories. Because auspex generates a
...
the table have no first-class `--managed` target in auspex; use their documented
...
auspex binary path: moving the binary or changing baked-in arguments such as
...
owned or world-writable plugin sources. The generated package embeds auspex's
...
merge `auspex` into it without dropping other approved IDs. If introducing an
allowlist, enumerate every plugin that should continue to load. Add `auspex` to
the allowlist before running `openclaw plugins enable auspex`; otherwise the CLI
...
global discovery, preventing a workspace package with the same `auspex` ID from
...
plugins: {
  enabled: true,
  allow: [/* existing approved IDs */, "auspex"],
  load: {
    paths: [
      /* existing approved paths */
      "/absolute/config-root/extensions/auspex",
    ],
  },
  entries: {
    /* existing entries */
...
and manage that user's OpenClaw configuration declaratively. There is no auspex
...
1. Confirm OpenClaw v2026.7.1 or newer. Keep auspex's generated ten-second
...
3. Run `openclaw plugins inspect auspex --runtime --json`. Confirm the reported
...
4. For a pilot installed with `--emit events` or `--emit all`, trigger one benign
   covered action and confirm that its auspex record reaches the expected local
...
embedded absolute auspex path both exist inside that runtime.
...
| Kiro IDE / CLI v3 | The default file requires IDE 1.0.182+ or CLI 2.13.0+ launched with `kiro-cli --v3`. Confirm the entries are enabled in the IDE's Agent Hooks panel. `KIRO_HOME` relocates the CLI target only; when both roots are active, install and check the IDE's `~/.kiro/hooks/auspex.json` separately with `--settings`. |
| Goose | Confirm `auspex` is not listed in `disabledPlugins`. |
...
`auspex hook status` verifies only that auspex-owned configuration is present
...
3. With events enabled, perform a benign known action and confirm a local auspex
...
1. Place the final auspex binary at an administrator-controlled absolute path.
...
editing belongs to `auspex hook install`, not to that wrapper.
...
touches. Keep the binary admin-controlled. A `$HOME/.auspex/...` output remains
...
inventory, or managed upgrade and uninstall. auspex does not require a
background collector package for discovery or hook capture.

Suggested pilot layout:

```text
/usr/local/bin/auspex                # or /opt/auspex/bin/auspex
/var/log/auspex/agents.<user>.json   # root-owned discovery output
~/.auspex/live.ndjson                # per-user live hook record stream
```
...
resolve the active console user first and execute auspex with that user's
...
AUSPEX=/usr/local/bin/auspex
LOG_DIR=/var/log/auspex
...
sudo -u "$CONSOLE_USER" HOME="$USER_HOME" "$AUSPEX" agents --all --format json > "$tmp"
...
USER_HOME=$(dscl . -read "/Users/$CONSOLE_USER" NFSHomeDirectory | awk '{print $2}')
[ -n "$USER_HOME" ] || exit 0
HOOK_OUT="$USER_HOME/.auspex/live.ndjson"
...
for agent in $TARGET_AGENTS; do
  sudo -u "$CONSOLE_USER" HOME="$USER_HOME" "$AUSPEX" hook install --agent "$agent" \
    --emit all \
    --output file \
...
For a container or remote Gateway, install the package and auspex binary in that
...
artifact must be generated on the target OS from the same final absolute auspex
...
```sh
sudo -u "$CONSOLE_USER" HOME="$USER_HOME" "$AUSPEX" hook install --agent codex \
  --emit findings --emit indicators \
  --output file \
  --output-file "$USER_HOME/.auspex/findings.ndjson"
```
...
sudo -u "$CONSOLE_USER" HOME="$USER_HOME" "$AUSPEX" hook install --agent codex \
  ...
  --output file \
  --output http \
  --output-file "$USER_HOME/.auspex/live.ndjson" \
  --http-url https://ingest.example/auspex
```
...
```sh
auspex hook install --agent gemini --enforce --emit all \
  --rules-dir /etc/auspex/rules
```

Enforcement is available for rows marked **yes** in the
[coverage matrix](agent-coverage.md#matrix). auspex withholds its deny response
...
`C:\Program Files\auspex\auspex.exe`; hook config must not point at a binary the
monitored user can replace. Run user-level installs in each target user's
context. Run managed installs elevated:

```powershell
$Auspex = "C:\Program Files\auspex\auspex.exe"
& $Auspex hook install --agent claude --managed --emit all `
  --output-file '$HOME/.auspex/live.ndjson'
& $Auspex hook install --agent codex --managed --emit all `
  --output-file '$HOME/.auspex/live.ndjson'
```
...
auspex hook status --agent codex --managed
tail -f ~/.auspex/live.ndjson
auspex scan --path ~/.codex --emit findings --emit indicators
```

Windows PowerShell equivalents:

```powershell
.\auspex.exe agents --all
.\auspex.exe hook status --agent codex
