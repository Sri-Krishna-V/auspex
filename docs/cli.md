...
This page describes every `auspex` subcommand, its flags, and its output.
...
```
auspex agents [--all]                    report detected agents; --all includes absent agents
auspex scan [--agent NAME ... | --path FILE|DIR ...]
                                        scan agent artifacts; emit records (NDJSON)
auspex timeline [--agent NAME ... | --path FILE|DIR ...]
                                        reconstruct a per-session chronological view
auspex collect [--addr HOST:PORT]        receive OTLP/HTTP protobuf logs; emit records
auspex ship --input-file F --http-url U  tail a local record file to an HTTP endpoint
auspex hook EVENT --agent NAME           live integration callback (normally not run by hand)
auspex hook install --agent NAME|all     install auspex's live integrations
auspex hook uninstall --agent NAME|all   remove auspex-owned live integrations
auspex hook status [--agent NAME|all]    inspect auspex-owned integration configuration
auspex rules check                       validate and compile rules; run companion tests
auspex rules list                        list effective compiled rule ids
auspex rules test --fixture FILE         evaluate rules against NDJSON events
auspex case build <case-id> --from FILE  bundle findings, enforcement decisions, and cited events
auspex case verify DIR                   check a bundle against its manifest digests
auspex version                           print version and schema version
```

Every command supports `--help`; `auspex help <subcommand>` prints subcommand
help to stdout.
...
`bearer` → `AUSPEX_HTTP_TOKEN`, `hmac-sha256` → `AUSPEX_HTTP_HMAC_KEY`.
...
`sha256=<lowercase-hex-hmac>` to `X-Auspex-Signature` by default. With the
default timestamp header, `X-Auspex-Timestamp` contains Unix seconds and the
...
retention, keep file output and let a log forwarder ship it. auspex does not
rotate files itself.
...
auspex scan --path ~/.claude/projects --path ~/.codex --path ~/.gemini

# stream HMAC-signed records to an HTTP ingest endpoint (key from the environment)
AUSPEX_HTTP_HMAC_KEY=... auspex scan \
  --output http --http-url https://ingest.example/auspex \
...
the source does not provide them; auspex does not infer these values.

Finding `timestamp` is the matched event's activity time (the completing event
for a sequence); `detected_at` is when auspex created the finding. `timestamp`
is omitted when that event has no valid timestamp.

### The record stream

auspex writes typed NDJSON streams. Each record carries a `record_type` (`event`,
...
```
auspex timeline --agent claude
auspex timeline --path ~/.claude/projects
auspex timeline --path ~/.claude/projects --path ~/.codex --format json
```
...
```
auspex collect
auspex collect --addr 127.0.0.1:4318 --emit all
```

Claude Code, Codex, Gemini CLI, Qwen Code, and OpenCode can emit OTLP logs that `collect`
normalizes. None points at auspex by default, so configure the logs exporter
explicitly:
...
- **Gemini CLI** — defaults to gRPC on `:4317`, which auspex does not serve. Set
  `otlpProtocol=http` (in `~/.gemini/settings.json` telemetry settings or via the
  CLI flags) and point the OTLP endpoint at auspex's `http://127.0.0.1:4318`.
...
OpenCode emits bare names such as `user_prompt` and `tool_result`. auspex matches
...
in traces, so use auspex's native OpenClaw plugin for live actions or a full
collector for that telemetry.

## ship

`ship` is an optional forwarder for hosts without an existing log shipper. It
tails a auspex NDJSON file and sends batches to an HTTP endpoint outside the
...
--http-sig-header NAME       HMAC signature header (default X-Auspex-Signature)
--http-timestamp-header NAME signed timestamp header; empty signs the bare body
                             (default X-Auspex-Timestamp)
--http-gzip                  gzip the POST body
--http-allow-insecure        allow plain http to non-loopback hosts
```
...
auspex hook install --agent codex --emit all \
  --output file --output-file ~/.auspex/records.ndjson

AUSPEX_HTTP_TOKEN=... auspex ship \
  --input-file ~/.auspex/records.ndjson \
...
`auspex hook EVENT` is the callback written by `hook install`; operators do not
...
or state, or output error handled by auspex suppresses its deny response;
unavailable sequence state does not suppress an independent stateless deny.
OpenClaw's generated wrapper likewise returns no block when the auspex child
...
one action-level `record_type:"enforcement"` record. It carries auspex's
...
On `auspex hook`, stdout is reserved for the agent's control response, so
...
`auspex hook install` wires the live handler into an agent's hook integration (and
...
on every auspex callback rather than inheriting a potentially ten-minute agent
...
`$HOME/.auspex/findings.ndjson`; selecting events or indicators changes the
default to `$HOME/.auspex/records.ndjson`. Use `--output-file PATH` to change
that file, or
...
installed hook reads `AUSPEX_HTTP_TOKEN` or `AUSPEX_HTTP_HMAC_KEY` from its
...
the current operating system's quoting and the generator's absolute auspex
...
`${OPENCLAW_HOME:-~}/.openclaw`; auspex appends `extensions/auspex/`. This
...
standalone auspex process. OpenClaw projects the concrete state/config paths
only inside its own invocation; exporting `OPENCLAW_PROFILE` alone does not do
that. Pass the profile's `OPENCLAW_STATE_DIR` to auspex, or use `--settings` for
...
auspex `--managed` target. `status` validates the package files and checksum,
...
auspex hook install --agent codex --emit all --output-file ~/.auspex/codex.ndjson
auspex hook install --agent codex --managed --emit all \
  --output-file '$HOME/.auspex/live.ndjson'
auspex hook install --agent openclaw --rules-dir /etc/auspex/rules --enforce
auspex hook install --agent openhands --settings /repo/.openhands/hooks.json
auspex hook status --agent codex
auspex hook uninstall --agent claude
```
...
```
auspex rules check --rules-dir ./my-rules
auspex rules list
auspex rules test --fixture events.ndjson --require-match
auspex rules test --fixture positive.ndjson --expect secrets.agent_read_env
auspex rules test --fixture negative.ndjson --expect-none
```

## case bundles

`case build` curates captured record streams into a portable `case.auspex`
...
-o, --out DIR                bundle directory to create (default case.auspex;
                             must not exist)
...
```
auspex scan --case-id inv-42 --emit all --output file --output-file records.ndjson
auspex case build inv-42 --from records.ndjson -o inv-42.auspex
auspex case verify inv-42.auspex
```
...
`auspex version` prints the tool version and the record schema version
