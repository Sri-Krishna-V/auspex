...
auspex embeds the rules below. They are enabled for detection and do not block
...
`auspex rules list` to print the effective rule IDs after operator overrides.
...
- `tamper.detector_state_write` - a command or structured action targets the
  default per-user `$HOME/.auspex` state directory.
