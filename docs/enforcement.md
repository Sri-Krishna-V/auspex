...
auspex hooks monitor by default. Enforce mode is an explicit opt-in that lets a
...
3. auspex validates and normalizes the payload, evaluates rules, and emits
   operator records.
4. On a clean enforced match, auspex returns that host's native deny response.
...
auspex does not execute or cancel the tool itself. A structured deny field or
...
```sh
auspex rules check --rules-dir /etc/auspex/rules
auspex hook install --agent codex --managed --rules-dir /etc/auspex/rules --enforce
```

The same-id operator rule replaces the embedded definition as a whole; omitted
fields are not inherited. Protect that directory like the auspex binary and
managed hook configuration.

auspex exposes only observe and deny effects. Agent hosts do not share portable
...
auspex recognizes explicit `-WhatIf` and a statically visible
...
| auspex decision | Hook response | Host behavior |
|---|---|---|
...
encodings of the normal host path, not additional auspex rule effects.

Fail-open means auspex withholds its deny response. It does **not** guarantee
...
some hook launch failures and non-zero exits deny even when auspex did not
produce a decision.
...
auspex routes available operational diagnostics there as diagnostic records
...
auspex requests a deny only when all applicable checks complete cleanly:
...
suppress the auspex deny. If sequence state is unavailable, correlation is
...
auspex emits no deny response and reports a diagnostic.
...
block when the auspex child fails or reaches its internal nine-second deadline,
...
strictly above nine seconds (the generated value is ten seconds). auspex
evaluates the tool
...
Host protocols and tool names can change independently. auspex keeps native
...
intentionally suppresses auspex's deny.
