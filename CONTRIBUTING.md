...
auspex is a single Go module and binary. Contributions should preserve its
...
- Report vulnerabilities through the private channel in [SECURITY.md](SECURITY.md),
  not a public issue.
- For a bug, include the output of `auspex version`, the operating system and
...
go vet ./...
go test -race ./...
go build -buildvcs=false ./cmd/auspex
golangci-lint run
...
- **Observe and fail open by default.** auspex blocks only after an operator
  enables enforce mode. A auspex error, panic, or uncertain result suppresses
...
- In `cmd/auspex`, scan, timeline, and case files must not import the monitoring
...
and unclassified non-test files in `cmd/auspex`. Raise any necessary boundary
