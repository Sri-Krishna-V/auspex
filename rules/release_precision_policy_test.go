...
func TestReleasePrecisionSecretAndStatePolicy(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"repository Auspex fixture stays quiet", write("/repo/testdata/.auspex/state.db"), ""},
		{"home-shaped Auspex command fixture stays quiet", cmd("truncate -s 0 /repo/home/dev/.auspex/state.db"), ""},
		{"active Auspex state write", write("/home/dev/.auspex/state.db"), "tamper.detector_state_write"},
		{"Auspex state PowerShell WhatIf still records intent", cmd("Remove-Item ~/.auspex/state.db -WhatIf"), "tamper.detector_state_write"},
		{"auspex state path used as PowerShell value stays quiet", cmd(`Set-Content -Path /tmp/note -Value '~/.auspex/state.db'`), ""},
		{"auspex state is the named PowerShell path operand", cmd(`Set-Content -LiteralPath ~/.auspex/state.db -Value x`), "tamper.detector_state_write"},
		{"auspex state is the PowerShell copy source stays quiet", cmd(`Copy-Item ~/.auspex/state.db /tmp/state.db`), ""},
		{"auspex state is the named PowerShell copy source stays quiet", cmd(`Copy-Item -Path ~/.auspex/state.db /tmp/state.db`), ""},
		{"auspex state is the named PowerShell copy destination", cmd(`Copy-Item /tmp/state.db -Destination ~/.auspex/state.db`), "tamper.detector_state_write"},
		{"moving auspex state away is tampering", cmd("mv ~/.auspex/state.db /tmp/state.db"), "tamper.detector_state_write"},
		{"moving auspex state away with PowerShell is tampering", cmd("Move-Item -Path ~/.auspex/state.db -Destination /tmp/state.db"), "tamper.detector_state_write"},
		{"quoted state example cannot suppress a real mutation", cmd(`truncate -s 0 ~/.auspex/state.db; echo "printf example > ~/.auspex/state.db"`), "tamper.detector_state_write"},
	})
