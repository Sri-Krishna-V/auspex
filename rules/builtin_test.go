...
func TestCommandRulesAcceptOnlyOTelCompletionRecords(t *testing.T) {
	...
	cases := []struct {
		...
		{"truncate -s 0 ~/.auspex/state.db", "tamper.detector_state_write"},
...
func TestExecDestructiveRecursiveDelete(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"rm rf tmp dir does not fire", cmd("rm -rf /tmp/auspex-test"), ""},
		...
		{"cmd rmdir temp subtree does not fire", cmd(`rmdir /s /q C:\temp\auspex-test`), ""},
