...
func TestEgressChainsSharePredicate(t *testing.T) {
	...
	negative := []struct {
		...
		{"curl user agent", cmd("curl --user-agent auspex https://docs.example/install")},
...
func TestTamperRules(t *testing.T) {
	...
	runCases(t, eng, []ruleCase{
		...
		{"opencode plugin write", write("/Users/dev/.config/opencode/plugins/auspex.ts"), "tamper.agent_config_write"},
		{"opencode plugin suffix match is intentional", write("/repo/opencode/plugins/auspex.ts"), "tamper.agent_config_write"},
		{"hermes config write", write("/Users/dev/.hermes/config.yaml"), "tamper.agent_config_write"},
		{"copilot cli user hook write", write("/Users/dev/.copilot/hooks/auspex-cli.json"), "tamper.agent_config_write"},
		{"copilot policy hook write", write("/etc/github-copilot/policy.d/auspex.json"), "tamper.agent_config_write"},
		{"vscode user hook write", write("/Users/dev/.copilot/hooks/auspex.json"), "tamper.agent_config_write"},
		{"vscode project hook write", write("/repo/.github/hooks/auspex.json"), "tamper.agent_config_write"},
		...
		{"opencode custom config plugin write", write("D:/managed/config/plugins/auspex.ts"), "tamper.agent_config_write"},
		{"grok managed hook write", write("/Users/dev/.grok/hooks/auspex-endpoint.json"), "tamper.agent_config_write"},
		{"kiro default hook write", write("/Users/dev/.kiro/hooks/auspex.json"), "tamper.agent_config_write"},
		{"relocated auspex hook write", write("/opt/agent-profile/hooks/auspex.json"), "tamper.agent_config_write"},
		...
		{"Pi default extension write", write("/Users/dev/.pi/agent/extensions/auspex.ts"), "tamper.agent_config_write"},
		...
		{"OpenClaw managed manifest write", write("/Users/dev/.openclaw/extensions/auspex/openclaw.plugin.json"), "tamper.agent_config_write"},
		{"OpenClaw managed package write", write("/Users/dev/.openclaw/extensions/auspex/package.json"), "tamper.agent_config_write"},
		{"OpenClaw managed entrypoint delete", model.Event{EventType: model.EventFileDelete, FilePath: "C:/Users/dev/.openclaw/extensions/auspex/index.js"}, "tamper.agent_config_write"},
		{"OpenClaw named profile package write", write("/Users/dev/.openclaw-corp_1/extensions/auspex/package.json"), "tamper.agent_config_write"},
		{"OpenClaw relocated package write", write("/srv/gateway-state/extensions/auspex/index.js"), "tamper.agent_config_write"},
		{"OpenClaw hidden relocated package write", write("/srv/.gateway-state/extensions/auspex/index.js"), "tamper.agent_config_write"},
		{"OpenClaw managed plugin directory delete", model.Event{EventType: model.EventFileDelete, FilePath: "/Users/dev/.openclaw/extensions/auspex"}, "tamper.agent_config_write"},
		{"OpenClaw relocated plugin directory delete", model.Event{EventType: model.EventFileDelete, FilePath: "/srv/gateway-state/extensions/auspex/"}, "tamper.agent_config_write"},
		...
		{"OpenClaw explicit legacy-named state package write", write("/Users/dev/.clawdbot/extensions/auspex/index.js"), "tamper.agent_config_write"},
		...
		{"Auggie user wrapper write", write("/Users/dev/.augment/hooks/auspex--installed-by=auspex-pre_tool.sh"), "tamper.agent_config_write"},
		{"Auggie Linux managed settings write", write("/etc/augment/settings.json"), "tamper.agent_config_write"},
		{"Auggie Windows managed wrapper write", write("C:/ProgramData/Augment/hooks/auspex--installed-by=auspex-session_end.ps1"), "tamper.agent_config_write"},
		{"Goose manifest write", write("/Users/dev/.agents/plugins/auspex/plugin.json"), "tamper.agent_config_write"},
		{"Goose hooks write", write("/Users/dev/.agents/plugins/auspex/hooks/hooks.json"), "tamper.agent_config_write"},
		{"Kilo plugin write", write("/Users/dev/.config/kilo/plugin/auspex.ts"), "tamper.agent_config_write"},
		...
		{"Windows Copilot policy write", write("c:/programdata/github/copilot/policy.d/auspex.json"), "tamper.agent_config_write"},
		...
		{"install Claude managed settings", cmd("sudo install -m 0644 /tmp/auspex.json '/Library/Application Support/ClaudeCode/managed-settings.d/auspex.json'"), "tamper.agent_config_write"},
		{"PowerShell copy Devin hooks", cmd(`Copy-Item C:\tmp\hooks.v1.json $HOME\.devin\hooks.v1.json`), "tamper.agent_config_write"},
		{"move Grok hook config", cmd("mv /tmp/auspex-endpoint.json ~/.grok/hooks/auspex-endpoint.json"), "tamper.agent_config_write"},
		{"remove Kiro hook config", cmd("rm -f ~/.kiro/hooks/auspex.json"), "tamper.agent_config_write"},
		...
		{"remove Pi extension", cmd("rm -f ~/.pi/agent/extensions/auspex.ts"), "tamper.agent_config_write"},
		{"edit Kimi hooks", cmd("sed -i 's/PreToolUse/Stop/' ~/.kimi-code/config.toml"), "tamper.agent_config_write"},
		{"edit OpenClaw default config", cmd("sed -i 's/auspex/disabled/' ~/.openclaw/openclaw.json"), "tamper.agent_config_write"},
		{"edit OpenClaw named profile config", cmd("sed -i 's/auspex/disabled/' ~/.openclaw-production/openclaw.json"), "tamper.agent_config_write"},
		{"remove OpenClaw managed manifest", cmd("rm -f ~/.openclaw/extensions/auspex/openclaw.plugin.json"), "tamper.agent_config_write"},
		{"copy over OpenClaw managed package", cmd("cp /tmp/package.json ~/.openclaw/extensions/auspex/package.json"), "tamper.agent_config_write"},
		{"copy over OpenClaw relocated package", cmd("cp /tmp/index.js /srv/gateway-state/extensions/auspex/index.js"), "tamper.agent_config_write"},
		{"remove OpenClaw explicit legacy-named package", cmd("rm -f ~/.clawdbot/extensions/auspex/index.js"), "tamper.agent_config_write"},
		{"remove OpenClaw managed plugin directory", cmd("rm -rf ~/.openclaw/extensions/auspex"), "tamper.agent_config_write"},
		{"remove OpenClaw relocated plugin directory with trailing slash", cmd("rm -rf /srv/gateway-state/extensions/auspex/"), "tamper.agent_config_write"},
		{"PowerShell remove OpenClaw managed plugin directory", cmd("Remove-Item -Recurse $HOME\\.openclaw\\extensions\\auspex"), "tamper.agent_config_write"},
		{"move OpenClaw managed entrypoint out", cmd("mv ~/.openclaw/extensions/auspex/index.js /tmp/index.js.disabled"), "tamper.agent_config_write"},
		{"move OpenClaw managed plugin directory out", cmd("mv ~/.openclaw/extensions/auspex /tmp/auspex.disabled"), "tamper.agent_config_write"},
		{"PowerShell move OpenClaw package out", cmd("Move-Item $HOME\\.openclaw\\extensions\\auspex\\package.json C:\\tmp\\package.json.disabled"), "tamper.agent_config_write"},
		{"edit OpenClaw relocated conventional config", cmd("sed -i 's/auspex/disabled/' /srv/gateway-state/openclaw.json"), "tamper.agent_config_write"},
		...
		{"PowerShell overwrite OpenClaw managed entrypoint", cmd("Set-Content $HOME\\.openclaw\\extensions\\auspex\\index.js disabled"), "tamper.agent_config_write"},
		...
		{"remove Auggie wrapper", cmd("rm -f ~/.augment/hooks/auspex--installed-by=auspex-stop.sh"), "tamper.agent_config_write"},
		{"remove Goose manifest", cmd("rm -f ~/.agents/plugins/auspex/plugin.json"), "tamper.agent_config_write"},
		{"remove Kilo plugin", cmd("rm -f ~/.config/kilo/plugin/auspex.ts"), "tamper.agent_config_write"},
		...
		{"copy Pi extension", cmd("cp /tmp/auspex.ts ~/.pi/agent/extensions/auspex.ts"), "tamper.agent_config_write"},
		...
		{"copy Goose hooks", cmd("cp /tmp/hooks.json ~/.agents/plugins/auspex/hooks/hooks.json"), "tamper.agent_config_write"},
		{"copy Kilo plugin", cmd("cp /tmp/auspex.ts ~/.config/kilo/plugin/auspex.ts"), "tamper.agent_config_write"},
		...
		{"relocated Pi directory is not guessed", write("/opt/pi-agent/extensions/auspex.ts"), ""},
		...
		{"OpenClaw plugin backup untouched", write("/Users/dev/.openclaw/extensions/auspex/index.js.bak"), ""},
		{"OpenClaw plugin name lookalike untouched", write("/srv/gateway-state/extensions/auspex-extra/index.js"), ""},
		{"OpenClaw nested entrypoint lookalike untouched", write("/srv/gateway-state/extensions/auspex/lib/index.js"), ""},
		...
		{"quoted Pi redirect example stays quiet", cmd(`echo "printf x > ~/.pi/agent/extensions/auspex.ts"`), ""},
		{"copying Pi extension out stays quiet", cmd("cp ~/.pi/agent/extensions/auspex.ts /tmp/auspex.ts"), ""},
		...
		{"quoted relocated OpenClaw plugin redirect stays quiet", cmd("echo \"printf x > /srv/gateway-state/extensions/auspex/index.js\""), ""},
		{"copying OpenClaw entrypoint out stays quiet", cmd("cp ~/.openclaw/extensions/auspex/index.js /tmp/index.js"), ""},
		{"copying OpenClaw plugin directory out stays quiet", cmd("cp -R ~/.openclaw/extensions/auspex /tmp/auspex-backup"), ""},
		{"PowerShell copying OpenClaw package out stays quiet", cmd("Copy-Item $HOME\\.openclaw\\extensions\\auspex\\package.json C:\\tmp\\package.json"), ""},
		{"moving OpenClaw package backup out stays quiet", cmd("mv ~/.openclaw/extensions/auspex/package.json.bak /tmp/package.json.bak"), ""},
		{"removing OpenClaw config backup stays quiet", cmd("rm -f /srv/gateway-state/openclaw.json.bak"), ""},
		{"removing OpenClaw plugin directory lookalike stays quiet", cmd("rm -rf /srv/gateway-state/extensions/auspex-extra"), ""},
		...
		{"auspex state write", write("/Users/dev/.auspex/state.db"), "tamper.detector_state_write"},
		{"Windows auspex state write", write("C:/Users/dev/.auspex/state.db"), "tamper.detector_state_write"},
		{"rm of auspex state", cmd("rm -rf ~/.auspex/"), "tamper.detector_state_write"},
		{"sudo rm of auspex state", cmd("sudo rm -f /Users/dev/.auspex/state.db"), "tamper.detector_state_write"},
		{"PowerShell remove auspex state", cmd(`Remove-Item -Recurse $HOME\.auspex`), "tamper.detector_state_write"},
		{"truncate auspex state", cmd("truncate -s 0 ~/.auspex/state.db"), "tamper.detector_state_write"},
		{"overwrite auspex state", cmd("printf x > ~/.auspex/state.db"), "tamper.detector_state_write"},
		{"tee auspex state", cmd("printf x | tee ~/.auspex/state.db"), "tamper.detector_state_write"},
		{"copy over auspex state", cmd("cp /tmp/state.db ~/.auspex/state.db"), "tamper.detector_state_write"},
		{"PowerShell clear auspex state", cmd(`Clear-Content $HOME\.auspex\state.db`), "tamper.detector_state_write"},
		{"sqlite3 cannot mutate the bbolt state store", cmd(`sqlite3 ~/.auspex/state.db 'delete from sequences'`), ""},
		{"quoted rm mention stays silent", cmd(`echo "rm -rf ~/.auspex/state.db"`), ""},
		{"quoted overwrite mention stays silent", cmd(`echo "printf x > ~/.auspex/state.db"`), ""},
		{"unrelated rm untouched", cmd("rm -rf node_modules"), ""},
		{"reading auspex state stays quiet", cmd("cat ~/.auspex/state.db"), ""},
		{"copying auspex state out stays quiet", cmd("cp ~/.auspex/state.db /tmp/state.backup.db"), ""},
		{"SQLite read of auspex state stays quiet", cmd(`sqlite3 ~/.auspex/state.db 'select * from sequences'`), ""},
